...
export function OzzylAIChatWidget() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!Number.isFinite(id) || id <= 0) {
        throw new Error((data?.error as string) || 'Registration failed: invalid visitor ID');
      }
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((data.error as string) || 'Failed to send message');
    ...
    <>
      ...
      {!isOpen && (
        <>
          ...
          {showGreeting && (
            <div
              className="fixed bottom-24 right-6 z-50 max-w-[280px] flex items-start gap-4 p-4 rounded-2xl cursor-pointer hover:scale-105 transition-transform origin-bottom-right shadow-2xl animate-fadeInUp"
              style={{
          ...
          <button
            onClick={() => setIsOpen(true)}
            className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all duration-300 group animate-fadeInUp"
            style={{
      ...
      {isOpen && (
        <div
          className="fixed bottom-6 right-6 z-50 w-[380px] max-w-[calc(100vw-48px)] h-[600px] max-h-[calc(100vh-100px)] flex flex-col overflow-hidden rounded-2xl shadow-2xl shadow-black/80 animate-fadeInUp"
          style={{
          ...
          <div className="flex items-center justify-between px-5 py-4 bg-[#0A2F25]/20 border-b border-[#006A4E]/20 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#006A4E] to-[#004D38] p-0.5 shadow-lg shadow-[#006A4E]/20">
                  <img
                    src={ASSETS.brand.logoSmallBlack}
                    alt="Ozzyl"
