...
export function LottieIcon({ src, size = 24, width, height, loop = true, autoplay = true, playOnHover = false, className = '', speed = 1, lazy = false, ariaLabel, }) {
    const lottieRef = useRef(null);
    const containerRef = useRef(null);
    const [isInView, setIsInView] = useState(!lazy);
    const [animationData, setAnimationData] = useState(typeof src === 'object' ? src : null);
    ...
    if (!animationData) {
        ...
        return (_jsx("div", { ref: containerRef, style: { width: finalWidth, height: finalHeight }, className: `bg-white/5 rounded animate-pulse ${className}`, "aria-label": ariaLabel, title: "Loading animation..." }));
    }
    console.log('[Lottie] Rendering animation:', typeof src === 'string' ? src : 'inline');
    return (_jsx("div", { ref: containerRef, style: { width: finalWidth, height: finalHeight }, className: `inline-flex items-center justify-center ${className}`, onMouseEnter: () => playOnHover && setIsHovered(true), onMouseLeave: () => playOnHover && setIsHovered(false), role: "img", "aria-label": ariaLabel, children: _jsx(Lottie, { lottieRef: lottieRef, animationData: animationData, loop: loop, autoplay: shouldPlay, style: { width: finalWidth, height: finalHeight } }) }));
}
