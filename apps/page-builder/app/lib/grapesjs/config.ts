...
/**
 * Compress Image using OffscreenCanvas (non-blocking) with fallback to regular Canvas
...
const compressImage = async (file: File): Promise<File> => {
  ...
  return new Promise((resolve) => {
    ...
    img.onload = async () => {
      ...
      try {
        // Try OffscreenCanvas first (non-blocking, better performance)
        if (typeof OffscreenCanvas !== 'undefined') {
          ...
          if (ctx) {
            ...
            // Convert to blob using OffscreenCanvas (runs off main thread)
            ...
            console.log(
              `[OffscreenCanvas] Converted to WebP: ${(file.size / 1024).toFixed(2)}KB -> ${(compressedFile.size / 1024).toFixed(2)}KB`
            );
        ...
        // Fallback to regular Canvas (blocks main thread but widely supported)
        ...
        canvas.toBlob(
          (blob) => {
            ...
            if (blob) {
              ...
              console.log(
                `[Canvas] Converted to WebP: ${(file.size / 1024).toFixed(2)}KB -> ${(compressedFile.size / 1024).toFixed(2)}KB`
              );
              resolve(compressedFile);
            } else {
              resolve(file);
        ...
        console.warn('[compressImage] Error during compression, returning original:', err);
        URL.revokeObjectURL(url);
        resolve(file);
      }
    };
