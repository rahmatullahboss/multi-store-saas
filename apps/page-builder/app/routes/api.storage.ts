...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  ).bind(parseInt(pageId), user.storeId).first<LandingPage>();
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const publish = new URL(request.url).searchParams.get('publish') === 'true' || 
                    data.publish === true || data.isPublished === true;
    ...
    if (pageId && !slug) {
      const existingPage = await db.prepare(
        'SELECT slug FROM landing_pages WHERE id = ? AND store_id = ? LIMIT 1'
      ).bind(parseInt(pageId), user.storeId).first<{ slug: string }>();
