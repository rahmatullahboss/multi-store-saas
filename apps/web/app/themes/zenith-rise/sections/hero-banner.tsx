...
export const schema: SectionSchema = {
  ...
  settings: [
    ...
    },
    {
      ...
      id: 'subheading',
      label: 'Subheading',
      default: 'The ultimate platform for modern creators. Scale your business with powerful tools designed for growth.',
    },
    {
      type: 'text',
      id: 'cta_label',
...
export default function ZenithHero({ context, settings }: SectionComponentProps) {
  const { getLink } = context;

  const heading = settings.heading as string;
  const titleHighlight = settings.title_highlight as string;
  ...
  return (
    <div className="relative overflow-hidden min-h-[90vh] flex items-center justify-center pt-20 pb-20 bg-slate-950">
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="text-center lg:text-left z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 animate-fade-in-up delay-300">
            <Link 
              to={getLink(settings.cta_link as string || '/signup')}
        ...
        <div className="relative group animate-fade-in-up delay-400 lg:block hidden">
           <div className="relative z-10 rounded-2xl overflow-hidden shadow-2xl shadow-indigo-500/20 border border-slate-700 bg-slate-900/50 backdrop-blur-sm transform transition-transform duration-700 hover:rotate-1 hover:scale-[1.02]">
              {settings.image ? (
                <img src={settings.image as string} alt="App Preview" className="w-full h-auto object-cover" />
              ) : (
