...
export const NEWSLETTER_AI_SCHEMA: AISchema = {
  ...
  properties: {
    ...
    successMessage: {
      type: 'string',
      description: 'Message shown after success',
      maxLength: 100,
      aiAction: 'enhance',
    },
  },
...
function NewsletterSectionBase({ settings, theme, storeId }: NewsletterSectionProps) {
  ...
  // Default to 'large' padding if not specified for better visual spacing
  const paddingTopValue = settings.paddingTop || 'large';
  const paddingBottomValue = settings.paddingBottom || 'large';

  const paddingTop =
    paddingTopValue === 'large'
      ? 'pt-20'
      : paddingTopValue === 'medium'
        ? 'pt-12'
        : paddingTopValue === 'small'
          ? 'pt-8'
          : 'pt-0';
  const paddingBottom =
    paddingBottomValue === 'large'
      ? 'pb-20'
      : paddingBottomValue === 'medium'
        ? 'pb-12'
        : paddingBottomValue === 'small'
          ? 'pb-8'
          : 'pb-0';
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {settings.subheading && (
          <p className="opacity-70 mb-6 max-w-md mx-auto">{settings.subheading}</p>
        )}
        ...
        >
          ...
          <div className="flex-1 relative">
            <input
              ...
              className="w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-0"
              style={{
                color: theme.text || theme.primary,
                borderColor: `${theme.primary || theme.text}20`,
