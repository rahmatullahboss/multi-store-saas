      ...
      return (
        <div className="relative">
          {settings.image && (
            <img 
              src={settings.image as string} 
              alt={settings.heading as string || 'Slide'} 
          ...
          />
          <div className={`absolute inset-0 flex flex-col justify-center p-8 ${settings.textPosition === 'center' ? 'items-center text-center' : ''} ${settings.textPosition === 'right' ? 'items-end text-right' : 'items-start'}`}>
            {settings.heading && (
            ...
            {settings.buttonText && (
              <a 
                href={settings.buttonLink as string || '#'}
      ...
      return (
        <div className="text-center p-6">
          ...
          {settings.description && (
            <p className="text-gray-600 text-sm">{settings.description as string}</p>
          )}
          {settings.link && (
            <a 
              href={settings.link as string} 
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          {settings.rating && (
            <div className="flex gap-1 mb-3">
              {Array.from({ length: settings.rating as number }).map((_, i) => (
                <span key={i} className="text-yellow-400">★</span>
              ))}
            </div>
          )}
          {/* Quote */}
          <p className="text-gray-700 italic mb-4">"{settings.quote as string}"</p>
          {/* Author */}
          <div className="flex items-center gap-3">
            {settings.avatar && (
              <img 
                src={settings.avatar as string} 
                alt={settings.author as string} 
                className="w-10 h-10 rounded-full object-cover"
              />
            )}
            <div>
              <p className="font-medium text-gray-900">{settings.author as string}</p>
              {settings.role && (
                <p className="text-sm text-gray-500">{settings.role as string}</p>
              )}
            </div>
          </div>
        </div>
