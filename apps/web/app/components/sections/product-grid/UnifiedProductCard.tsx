...
export function UnifiedProductCard({
  ...
  return (
    ...
    >
      ...
      <button
        className={`absolute top-2 right-2 w-8 h-8 rounded-full bg-white/80 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 transition-all hover:bg-white cursor-pointer z-10 ${isLuxury ? 'top-4 right-4 p-2.5 hover:scale-110' : ''}`}
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
      ...
      {isLuxury && showAddToCart && (
        <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 focus-within:opacity-100 focus-within:translate-y-0 translate-y-4 group-hover:translate-y-0 transition-all duration-300 z-10 pointer-events-none">
          <div className="pointer-events-auto">
            <AddToCartButton
              ...
              className="w-full px-6 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
              style={{
