...
export function BDShopPageWrapper({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: BDSHOP_BG, fontFamily: "'Inter', 'NotoSans', Arial, sans-serif" }}
    >
      ...
      <div style={{ backgroundColor: BDSHOP_NAVY_DARK }} className="hidden md:block">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-9 text-xs text-white/90">
          <div className="flex items-center gap-4">
            ...
            <span className="text-yellow-400 text-[10px] bg-yellow-400/20 px-2 py-0.5 rounded">
              Beta Version
            </span>
          </div>
          <div className="flex items-center gap-4">
      ...
      {/* Main Header */}
      <header
        style={{ backgroundColor: BDSHOP_CARD_BG }}
      ...
      >
        <div className="max-w-7xl mx-auto px-3 md:px-4 flex items-center h-14 md:h-16 gap-2 md:gap-4">
          ...
          >
            {mobileMenuOpen ? (
              <X className="h-6 w-6" style={{ color: BDSHOP_TEXT }} />
            ) : (
              <Menu className="h-6 w-6" style={{ color: BDSHOP_TEXT }} />
            )}
          </button>
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0">
            {logo ? (
              ...
              <div className="flex items-center gap-1">
                <div
                  className="h-8 w-8 md:h-10 md:w-10 rounded flex items-center justify-center"
        ...
        <div className="hidden md:block border-t" style={{ backgroundColor: BDSHOP_CARD_BG }}>
          <div className="max-w-7xl mx-auto px-4 flex items-center h-10 gap-6 text-sm">
            ...
            <Link to="/?sale=true" className="font-medium transition" style={{ color: '#EF4444' }}>
              Sale
            </Link>
            <Link
              to="/about"
        ...
        <nav className="bg-white border-b">
          <div className="max-w-7xl mx-auto px-4 py-2 md:py-3">
            <div className="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm overflow-x-auto">
              <Link to="/" className="text-gray-500 hover:text-blue-600 transition shrink-0">
                Home
              </Link>
              {breadcrumb.map((item, index) => (
                <span key={index} className="flex items-center gap-1.5 shrink-0">
                  ...
                  {item.href ? (
                    <Link to={item.href} className="text-gray-500 hover:text-blue-600 transition">
                      {item.label}
                    </Link>
                  ) : (
          ...
          <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-12 mb-8 pb-8 border-b border-white/10">
            <div className="md:w-1/3">
              ...
              <p className="text-white/70 text-xs md:text-sm leading-relaxed">
                Your premier destination for quality electronics and gadgets. We deliver excellence
                in every product.
              </p>
              {businessInfo?.phone && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 flex-1">
              ...
              <div>
                ...
                <ul className="space-y-2 text-xs md:text-sm">
                  <li>
                    <Link to="/faq" className="text-white/70 hover:text-white transition">
                      FAQ
                    </Link>
                  </li>
                  <li>
                    <Link to="/shipping" className="text-white/70 hover:text-white transition">
                      Shipping Info
                    </Link>
                  </li>
                  <li>
                    <Link to="/returns" className="text-white/70 hover:text-white transition">
                      Returns & Refunds
                    </Link>
                  </li>
                  <li>
                    <Link to="/track" className="text-white/70 hover:text-white transition">
                      Track Order
                    </Link>
                  </li>
                </ul>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-6 border-t border-white/10">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1.5">
                <span className="px-2 py-1 bg-white/10 rounded text-[10px] text-white/80">
                  bKash
                </span>
                <span className="px-2 py-1 bg-white/10 rounded text-[10px] text-white/80">
                  Nagad
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-50">
        <div className="flex items-center justify-around h-14">
          <Link to="/" className="flex flex-col items-center gap-0.5 py-1 px-3">
            ...
            <span className="text-[10px] font-medium" style={{ color: BDSHOP_TEXT_LIGHT }}>
              Home
            </span>
          </Link>
          <Link to="/" className="flex flex-col items-center gap-0.5 py-1 px-3">
