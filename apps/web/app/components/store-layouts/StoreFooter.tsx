      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-4 text-center md:text-left">
            <Link
              to="/"
              className={`text-xl font-bold inline-flex items-center gap-2 ${textColor}`}
            >
              {logo && <img src={logo} alt={storeName} className="h-8 w-8 object-contain" />}
              {storeName}
            </Link>
            <p className={`text-sm ${mutedColor}`}>{footerDescription}</p>
            <div className="flex gap-4 justify-center md:justify-start">
              {socialLinks?.facebook && (
                <a
                  ...
                  className={`${mutedColor} hover:text-blue-500 transition`}
                >
              ...
              {socialLinks?.instagram && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${mutedColor} hover:text-pink-500 transition`}
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              ...
              {socialLinks?.whatsapp && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${mutedColor} hover:text-green-500 transition`}
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              ...
              <ul className="space-y-2">
                {column.links.map((link) => (
                  <li key={`${column.title}-${link.label}-${link.url}`}>
                    <Link
                      to={link.url}
                      className={`text-sm ${mutedColor} ${hoverColor} transition-colors`}
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="text-center md:text-left">
            ...
            <ul className="space-y-3">
              {businessInfo?.email && (
                <li
                  className={`flex items-center justify-center md:justify-start gap-2 text-sm ${mutedColor}`}
                >
                  📧 {businessInfo.email}
                </li>
              )}
              ...
              {businessInfo?.address && (
                <li
                  className={`flex items-start justify-center md:justify-start gap-2 text-sm ${mutedColor}`}
                >
                  📍 {businessInfo.address}
                </li>
              )}
