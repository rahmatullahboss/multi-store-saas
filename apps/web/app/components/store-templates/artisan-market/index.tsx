...
import { ShoppingBasket, Search, Menu, X, Heart, Leaf, ChevronRight, Instagram, Facebook, MapPin, Phone, Mail, Home as HomeIcon, Grid3X3, User, ShoppingCart, MessageCircle } from 'lucide-react';
    ...
    <StoreConfigProvider config={config}>
      <WishlistProvider>
        <ClientOnly fallback={<SkeletonLoader />}>
          {() => (
            <div className="min-h-screen pb-16 md:pb-0" style={{ backgroundColor: ARTISAN_MARKET_THEME.background, fontFamily: "'Work Sans', sans-serif" }}>
      {/* Google Fonts */}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-50">
        <div className="flex items-center justify-around h-14">
          <Link to="/" className="flex flex-col items-center gap-0.5 py-1 px-3">
            <HomeIcon className="w-5 h-5" style={{ color: !currentCategory ? ARTISAN_MARKET_THEME.accent : ARTISAN_MARKET_THEME.muted }} />
            <span className="text-[10px] font-medium" style={{ color: !currentCategory ? ARTISAN_MARKET_THEME.accent : ARTISAN_MARKET_THEME.muted }}>Home</span>
          </Link>
          <button 
          ...
          >
            ...
            <span className="text-[10px] font-medium" style={{ color: ARTISAN_MARKET_THEME.muted }}>Browse</span>
          </button>
          <Link to="/cart" className="flex flex-col items-center gap-0.5 py-1 px-3 relative">
            <ShoppingCart className="w-5 h-5" style={{ color: ARTISAN_MARKET_THEME.muted }} />
            <span 
            ...
            <span className="text-[10px] font-medium" style={{ color: ARTISAN_MARKET_THEME.muted }}>Basket</span>
          </Link>
          {!isPreview && (
            <Link to="/auth/login" className="flex flex-col items-center gap-0.5 py-1 px-3">
              ...
              <span className="text-[10px] font-medium" style={{ color: ARTISAN_MARKET_THEME.muted }}>Account</span>
            </Link>
          )}
        </div>
      </nav>
      ...
      <Link to={`/product/${product.id}`} className="block relative aspect-square overflow-hidden">
        ...
        {isOnSale && (
          <div 
            ...
            style={{ backgroundColor: isFlashSale ? '#EF4444' : ARTISAN_MARKET_THEME.accent, color: 'white' }}
          >
      ...
      <div className="p-6">
        ...
        <Link to={`/product/${product.id}`}>
          <h3 
            ...
            style={{ fontFamily: "'Newsreader', serif", color: ARTISAN_MARKET_THEME.text, fontSize: '1.125rem' }}
          >
            {product.title}
          </h3>
        </Link>
