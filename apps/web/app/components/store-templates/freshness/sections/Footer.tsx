...
import { Mail, Phone, MapPin, Instagram, Facebook, Twitter, ShieldCheck, Truck, RotateCcw } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            <Link to="/" className="text-3xl font-bold italic" style={{ fontFamily: theme.fontHeading, color: theme.primary }}>
               {logo ? (
            ...
            <p className="text-gray-600 leading-relaxed">
              {footerConfig?.description || 'Your one-stop shop for fresh, organic, and healthy groceries delivered to your doorstep.'}
            </p>
            <div className="flex items-center gap-4">
              <a href="#" className="w-10 h-10 rounded-full flex items-center justify-center bg-gray-100 hover:bg-green-600 hover:text-white transition-all">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {validCategories.slice(0, 5).map((category) => (
                <li key={category}>
                  <Link to={`/?category=${encodeURIComponent(category)}`} className="text-gray-600 hover:text-green-600 font-medium transition-colors">
                    {category}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li><Link to="/about" className="text-gray-600 hover:text-green-600 font-medium transition-colors">About Freshness</Link></li>
              <li><Link to="/contact" className="text-gray-600 hover:text-green-600 font-medium transition-colors">Contact Us</Link></li>
              <li><Link to="/shipping" className="text-gray-600 hover:text-green-600 font-medium transition-colors">Shipping Guide</Link></li>
              <li><Link to="/privacy" className="text-gray-600 hover:text-green-600 font-medium transition-colors">Privacy Policy</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-4 text-gray-600">
                ...
                <span className="text-sm font-medium">{businessInfo?.address || 'Freshness Market, 101 Green Valley, Eco City'}</span>
              </li>
              <li className="flex items-center gap-4 text-gray-600">
                <Phone className="w-6 h-6 text-green-600 shrink-0" />
                <span className="text-sm font-medium">{businessInfo?.phone || '+880 1234567890'}</span>
              </li>
              <li className="flex items-center gap-4 text-gray-600">
                <Mail className="w-6 h-6 text-green-600 shrink-0" />
                <span className="text-sm font-medium">{businessInfo?.email || `hello@${storeName.toLowerCase().replace(/\s/g, '')}.com`}</span>
              </li>
            </ul>
          </div>
        </div>
          ...
          <div className="flex items-center gap-8">
            ...
            {(planType === 'free' || footerConfig?.showPoweredBy !== false) && (
              ...
              >
                ...
                <span className="font-bold tracking-tight text-base italic text-green-600">Ozzyl</span>
              </a>
            )}
          </div>
        </div>
