  ...
  return (
    <div className="md:hidden pb-24">
      ...
      <header className="flex items-center justify-between py-6">
        ...
        <button aria-label={t('notifications') || 'Notifications'} className="relative p-2.5 rounded-full bg-white shadow-soft border border-slate-100 hover:scale-105 transition-transform">
        ...
        <div>
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <button aria-label={t('previous') || 'Previous'} className="p-1.5 rounded-full border border-slate-200 hover:bg-slate-100 text-slate-500 transition-colors">
                <ChevronRight className="w-5 h-5 rotate-180" />
              </button>
              <button aria-label={t('next') || 'Next'} className="p-1.5 rounded-full border border-slate-200 hover:bg-slate-100 text-slate-500 transition-colors">
              ...
              {popularProducts.map((product) => (
                <div key={product.id} className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group cursor-pointer" onClick={() => navigate(`/products/${product.id}`)}>
                  <div className="aspect-square bg-slate-50 rounded-lg mb-4 overflow-hidden relative">
                    ...
                    <button aria-label={t('addToFavorites') || 'Add to favorites'} className="absolute top-2 right-2 p-1.5 bg-white/80 backdrop-blur-sm rounded-full text-slate-400 hover:text-red-500 transition-colors shadow-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button aria-label={t('add') || 'Add'} className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-primary hover:text-white transition-colors">
