...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Fetch variants for each test (Optimized N+1 query)
  const testIds = tests.map(test => test.id);

  let variantsByTestId: Record<number, typeof abTestVariants.$inferSelect[]> = {};
  if (testIds.length > 0) {
    ...
    variantsByTestId = allVariants.reduce((acc, variant) => {
...
export default function ABTestsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active':
        return <span className="px-2 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-full">চলমান</span>;
      case 'paused':
        return <span className="px-2 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full">বিরতি</span>;
      case 'concluded':
        return <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs font-medium rounded-full">সম্পন্ন</span>;
      default:
        // Treat others as draft/paused
        return <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded-full">খসড়া</span>;
    }
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {tests.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="p-4 bg-white rounded-xl border border-gray-200">
            <p className="text-sm text-gray-500">মোট টেস্ট</p>
          ...
          <div className="p-4 bg-white rounded-xl border border-gray-200">
            <p className="text-sm text-gray-500">মোট কনভার্সন</p>
      ...
      {tests.length === 0 ? (
        ...
        <div className="space-y-4">
          {tests.map(test => (
            <div
              key={test.id}
              className="p-5 bg-white rounded-xl border border-gray-200 hover:border-indigo-300 transition"
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {test.status === 'active' && (
                    <Form method="post">
                      ...
                      <button
                        type="submit"
                        disabled={isSubmitting}
                        className="flex items-center gap-1 px-3 py-1.5 bg-yellow-100 text-yellow-700 rounded text-sm hover:bg-yellow-200"
                      >
                        <Pause size={14} /> বিরতি
                      </button>
                    </Form>
                  ...
                  <Link
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 bg-indigo-100 text-indigo-700 rounded text-sm hover:bg-indigo-200"
                  >
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-xl border border-blue-200">
        <h4 className="font-semibold text-blue-800 mb-2">💡 A/B টেস্টিং কিভাবে কাজ করে?</h4>
