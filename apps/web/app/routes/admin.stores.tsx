...
export async function action({ request, context }: ActionFunctionArgs) {
  const db = context.cloudflare.env.DB;
  const { userId: adminId, userEmail: adminEmail } = await requireSuperAdmin(
    request,
  ...
  if (intent === 'softDelete') {
    ...
    const storeData = await drizzleDb
    ...
    if (kvNamespace && storeData.length > 0) {
      ...
      console.log(`[ADMIN_DELETE] Cache invalidated for store ${storeId} (${subdomain})`);
    }
            ...
            <tbody className="divide-y divide-white/5">
              {filteredStores.length === 0 ? (
                ...
                filteredStores.map((store) => (
                  <tr
                    key={store.id}
                    className={`hover:bg-slate-800/50 transition ${selectedStores.includes(store.id) ? 'bg-blue-500/10' : ''}`}
                  >
                    {/* Checkbox */}
                    {!showDeleted && (
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <Store
                              className={`w-5 h-5 ${store.isActive ? 'text-blue-400' : 'text-slate-500'}`}
                            />
                          </div>
                          <div>
                        ...
                        <div className="flex flex-wrap gap-1 items-center">
                          {store.tags?.map((tag) => (
                            <span
                              key={tag.id}
                              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-slate-700 text-slate-300 border border-slate-600"
                            >
                              {tag.tag}
                              {!showDeleted && (
                    ...
                    {/* Plan */}
                    <td className="px-4 py-4">{getPlanBadge(store.planType)}</td>
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className={`font-mono font-bold ${
                            (store.aiCredits || 0) > 100
                              ? 'text-emerald-400'
                              : (store.aiCredits || 0) > 20
                                ? 'text-amber-400'
                                : 'text-red-400'
                          }`}
                        >
                        ...
                        {showDeleted ? (
                          ...
                          <fetcher.Form
                            ...
                            onSubmit={(e) => {
                              if (
                                !confirm(
                                  `Are you sure you want to delete "${store.name}"? This can be undone from the Deleted Stores view.`
                                )
                              ) {
                                e.preventDefault();
                              }
              ...
              <div className="grid grid-cols-3 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300">
                    ...
                    {store.usage.productsLimit === Infinity ? '∞' : store.usage.productsLimit}
                  </p>
                </div>
              </div>
