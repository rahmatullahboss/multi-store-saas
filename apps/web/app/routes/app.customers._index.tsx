...
import { useLoaderData, Link, Form, useSubmit, useSearchParams, useNavigation } from '@remix-run/react';
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (segment !== 'all') {
    if (segment === 'vip') {
      // Example logic: VIP is hardcoded or relies on segment field
      // query.where(eq(customers.segment, 'vip')); 
      // OR calculate dynamic if not relying on field
      query.where(and(eq(customers.storeId, storeId), eq(customers.segment, 'vip')));
    } else if (segment === 'new') {
      query.where(and(eq(customers.storeId, storeId), eq(customers.segment, 'new')));
    } else if (segment === 'returning') {
      // query.where(and(eq(customers.storeId, storeId), gt(customers.totalOrders, 1)));
      // allow flexible query
    }
  }

  // Execute Query
  const allCustomers = await query.orderBy(desc(customers.createdAt)).limit(limit).offset(offset);
                ...
                {allCustomers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-gray-50/80 transition group">
                    <td className="px-6 py-4">
                      <input type="checkbox" className="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 opactiy-0 group-hover:opacity-100 transition-opacity" />
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Link to={`/app/customers/${customer.id}`} className="font-medium text-gray-900 hover:text-emerald-600">
