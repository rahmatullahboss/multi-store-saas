...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Get step counts (Optimized N+1 query)
  const automationIds = automations.map(a => a.id);

  let stepsByAutomationId: Record<number, typeof emailAutomationSteps.$inferSelect[]> = {};
  if (automationIds.length > 0) {
    ...
    stepsByAutomationId = allSteps.reduce((acc, step) => {
