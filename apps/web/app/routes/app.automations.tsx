...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Optimizing DB latency by extracting IDs and using a single batch `inArray` query
  // instead of N+1 database queries in Promise.all() loop
  const automationIds = automations.map(a => a.id);
  let automationsWithSteps = automations.map(a => ({ ...a, stepCount: 0, steps: [] as typeof allSteps }));
  let allSteps: { id: number; automationId: number; delayMinutes: number; templateId: string; subject: string; body: string; createdAt: Date; updatedAt: Date; }[] = [];
  ...
  if (automationIds.length > 0) {
    ...
    // Group steps by automationId in memory
