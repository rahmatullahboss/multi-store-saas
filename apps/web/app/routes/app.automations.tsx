...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Replace N+1 Promise.all query in loop with single batch inArray query
  const automationIds = automations.map(a => a.id);

  // Fetch all steps for these automations in one query
  ...
  // Group steps by automation ID in memory
  ...
  // Combine automations with their steps
