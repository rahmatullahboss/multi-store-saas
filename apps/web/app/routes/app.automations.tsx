...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Get step counts (Optimized N+1 query to a single query)
  const automationIds = automations.map((a) => a.id);

  let allSteps: typeof emailAutomationSteps.$inferSelect[] = [];
  ...
  const stepsByAutomationId = allSteps.reduce((acc, step) => {
    if (!acc[step.automationId]) acc[step.automationId] = [];
