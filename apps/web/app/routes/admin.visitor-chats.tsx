...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Use inArray to avoid N+1 queries and avoid hitting D1 batch limits.
  
  let visitorsWithData: any[] = [];
  const visitorIds = allVisitors.map(v => v.id);

  if (visitorIds.length > 0) {
    const messagesStats = await db
      .select({
          visitorId: visitorMessages.visitorId,
          count: sql<number>`count(*)`,
          latestMessageId: sql<number>`max(${visitorMessages.id})`,
      })
      .from(visitorMessages)
      .where(inArray(visitorMessages.visitorId, visitorIds))
      .groupBy(visitorMessages.visitorId);

    const latestMessageIds = messagesStats.map(s => s.latestMessageId).filter(Boolean);
    ...
    for (const stat of messagesStats) {
        statsMap.set(stat.visitorId, stat);
    }
    for (const msg of latestMessages) {
        latestMsgMap.set(msg.id, msg);
    }
    ...
    visitorsWithData = allVisitors.map(v => {
        ...
        if (stats?.latestMessageId) {
           latestMsg = latestMsgMap.get(stats.latestMessageId);
        }
        return {
           ...v,
