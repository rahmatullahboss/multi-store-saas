...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Batch N+1 Drizzle ORM Queries to avoid DB latency
  ...
  if (visitorIds.length > 0) {
    messagesData = await db
      .select({
        ...
        // Relies on SQLite's bare column aggregation trick
        lastMessage: visitorMessages.content,
        lastActive: sql`max(${visitorMessages.createdAt})`.mapWith(visitorMessages.createdAt),
  ...
  const messageMap = new Map(messagesData.map(d => [d.visitorId, d]));

  const visitorsWithData = allVisitors.map(v => {
    const data = messageMap.get(v.id);
    return {
      ...v,
