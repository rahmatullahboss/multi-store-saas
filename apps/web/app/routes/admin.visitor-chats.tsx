...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: Used inArray with GROUP BY to avoid N+1 queries.
  
  const visitorIds = allVisitors.map(v => v.id);

  let visitorStatsMap = new Map();
  if (visitorIds.length > 0) {
    const stats = await db
      .select({
        visitorId: visitorMessages.visitorId,
        count: sql<number>`count(*)`,
        lastMessage: visitorMessages.content, // bare column guaranteed to come from max row in SQLite
        lastActiveRaw: sql<number>`MAX(${visitorMessages.createdAt})`
      })
    ...
    stats.forEach(s => {
      visitorStatsMap.set(s.visitorId, {
        messageCount: s.count,
        lastMessage: s.lastMessage,
        lastActive: s.lastActiveRaw ? new Date(s.lastActiveRaw) : null,
      });
    });
  }

  const visitorsWithData = allVisitors.map(v => {
    const stat = visitorStatsMap.get(v.id);
    return {
        ...v,
