...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (visitorIds.length > 0) {
    // ⚡ Bolt Optimization: Replace N+1 queries with single batch queries using inArray
    const [allMessages, countResults] = await Promise.all([
      db
        .select()
    ...
    // Group messages by visitorId (since they are ordered desc by createdAt, the first one is the latest)
    for (const msg of allMessages) {
      if (msg.visitorId !== null && !messagesMap.has(msg.visitorId)) {
        messagesMap.set(msg.visitorId, msg);
      }
    }

    for (const res of countResults) {
      if (res.visitorId !== null) {
        countMap.set(res.visitorId, res.count);
      }
    }
  }

  const visitorsWithData = allVisitors.map(v => {
    const latestMessage = messagesMap.get(v.id);
    const msgCount = countMap.get(v.id) || 0;

    return {
      ...v,
      lastMessage: latestMessage?.content || 'No messages',
      lastActive: latestMessage?.createdAt || v.createdAt,
