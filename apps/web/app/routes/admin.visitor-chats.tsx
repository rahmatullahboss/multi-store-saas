...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Combine N+1 query loop into a single grouped query over an inArray of visitor IDs.
  // SQLite bare columns return the row value corresponding to the max aggregate.
  ...
  if (visitorIds.length > 0) {
    const aggregatedMessages = await db
      .select({
        ...
        latestCreatedAt: sql<number>`max(${visitorMessages.createdAt})`,
    ...
    aggregatedMessages.forEach(msg => {
      messagesMap.set(msg.visitorId, msg);
    });
  }

  const visitorsWithData = allVisitors.map((v) => {
    const msgData = messagesMap.get(v.id);
    return {
        ...v,
        lastMessage: msgData?.content || 'No messages',
        // Note: Drizzle aggregate max on integer timestamp returns numeric epoch in seconds, rehydrate to Date.
        lastActive: msgData?.latestCreatedAt ? new Date(msgData.latestCreatedAt * 1000) : v.createdAt,
        messageCount: msgData?.count || 0
    };
  });
