...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Replaced N+1 query loop with a single grouped query
  const visitorIds = allVisitors.map(v => v.id);
  
  const statsMap = new Map<number, { lastMessage: string, lastActive: Date | null, messageCount: number }>();
