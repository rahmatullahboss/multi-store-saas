...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Fix N+1 query problem by batching the message counts and latest messages
  // Impact: Reduces 50 sequential DB queries down to 1 bulk query, saving massive network latency
  // on Cloudflare D1. Uses SQLite's bare columns behavior to fetch the exact row's `content` matching `max(createdAt)`.
  const visitorIds = allVisitors.map((v) => v.id);
  
  const messageStats: Record<number, { count: number; lastMessage: string | null; lastActive: string | null }> = {};
