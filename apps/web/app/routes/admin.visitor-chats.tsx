...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // We use a single query with grouping to prevent N+1 connection/network latency on D1.
  ...
  if (allVisitors.length > 0) {
      ...
      const aggregates = await db.select({
          ...
          lastMessage: visitorMessages.content, // SQLite gets the bare column from max() row
      ...
      const aggMap = new Map(aggregates.map(a => [a.visitorId, a]));
      visitorsWithData = allVisitors.map(v => {
          const agg = aggMap.get(v.id);
          return {
              ...v,
              lastMessage: agg?.lastMessage || 'No messages',
              lastActive: agg?.lastActiveSeconds ? new Date(agg.lastActiveSeconds) : v.createdAt,
