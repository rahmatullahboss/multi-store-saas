...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Batch N+1 Drizzle ORM Queries optimization
  ...
  if (visitorIds.length > 0) {
    // 1. Get aggregate stats and latest message ID per visitor
    ...
    // 2. Fetch the actual latest messages
    const latestMessageIds = stats.map(s => s.latestId).filter(Boolean);
    const latestMessages = latestMessageIds.length > 0
      ? await db
          .select()
          .from(visitorMessages)
    ...
    // Create lookup maps
    const statsMap = new Map(stats.map(s => [s.visitorId, s]));
    const latestMessageMap = new Map(latestMessages.map(m => [m.id, m]));

    // Update the objects
    visitorsWithData.forEach(v => {
      const vStats = statsMap.get(v.id);
      if (vStats) {
        v.messageCount = vStats.count;
        const latestMsg = latestMessageMap.get(vStats.latestId);
        if (latestMsg) {
          v.lastMessage = latestMsg.content || 'No messages';
          v.lastActive = latestMsg.createdAt || v.createdAt;
        }
      }
    });
  }
