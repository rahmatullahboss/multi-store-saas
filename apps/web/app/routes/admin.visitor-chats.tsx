...
import { desc, sql, eq, inArray, max } from 'drizzle-orm';
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Fix N+1 queries. We extract all visitor IDs and use
  // a single grouped `inArray` query. With SQLite, an aggregate bare column
  // (like `visitorMessages.content` alongside `max()`) correctly returns the row matching the max value.
  // Impact: Reduces DB queries from potentially 101 to just 2, resolving Cloudflare D1 latency bottleneck.
  const visitorIds = allVisitors.map(v => v.id);
  const visitorStats = visitorIds.length > 0 ? await db
    .select({
      visitorId: visitorMessages.visitorId,
      count: sql<number>`count(*)`,
      lastMessageContent: visitorMessages.content,
      maxCreatedAt: max(visitorMessages.createdAt),
    })
    .from(visitorMessages)
    .where(inArray(visitorMessages.visitorId, visitorIds))
    .groupBy(visitorMessages.visitorId) : [];

  const statsMap = new Map(visitorStats.map(s => [s.visitorId, s]));

  const visitorsWithData = allVisitors.map(v => {
    const stats = statsMap.get(v.id);
    return {
        ...v,
        lastMessage: stats?.lastMessageContent || 'No messages',
        // Drizzle max() returns numeric timestamp in seconds when mode is 'timestamp' on Cloudflare D1/SQLite
        lastActive: stats?.maxCreatedAt ? new Date(Number(stats.maxCreatedAt) * 1000) : v.createdAt,
