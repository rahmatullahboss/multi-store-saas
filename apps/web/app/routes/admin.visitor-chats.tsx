...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Replace N+1 queries with a single batched query using GROUP BY
  ...
  if (visitorIds.length > 0) {
    const aggregatedData = await db
      .select({
        ...
        lastActive: sql<string | null>`MAX(${visitorMessages.createdAt})`,
    ...
    const dataMap = new Map(aggregatedData.map((d) => [d.visitorId, d]));
