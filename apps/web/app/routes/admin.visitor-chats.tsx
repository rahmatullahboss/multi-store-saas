...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Group independent queries to avoid N+1 using inArray and groupBy
  const aggregatedMessages = visitorIds.length > 0 ? await db
    .select({
  ...
  const messageMap = new Map(aggregatedMessages.map(m => [m.visitorId, m]));

  const visitorsWithData = allVisitors.map(v => {
    const m = messageMap.get(v.id);
    return {
      ...v,
      lastMessage: m?.lastMessage || 'No messages',
      lastActive: m?.lastActive ? new Date(m.lastActive) : v.createdAt,
