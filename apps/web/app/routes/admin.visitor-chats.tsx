...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (visitorIds.length > 0) {
    aggregatedMessages = await db
      .select({
        ...
        lastMessageContent: visitorMessages.content, // SQLite guarantees this is from the row with max(createdAt)
        lastActive: sql<number>`max(${visitorMessages.createdAt})` // Returns epoch ms in D1
      })
  ...
  const messagesByVisitorId = aggregatedMessages.reduce((acc, curr) => {
    acc[curr.visitorId] = curr;
    return acc;
  }, {} as Record<number, any>);

  const visitorsWithData = allVisitors.map((v) => {
    const data = messagesByVisitorId[v.id];
    return {
      ...v,
      lastMessage: data?.lastMessageContent || 'No messages',
