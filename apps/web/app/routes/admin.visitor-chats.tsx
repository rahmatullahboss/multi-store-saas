...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: Use a single query with grouping. Selecting a bare column (content)
  // alongside an aggregate (MAX) in SQLite guarantees the bare column comes from the row with the MAX value.
  const visitorIds = allVisitors.map(v => v.id);
  const visitorData = visitorIds.length > 0 ? await db
    .select({
  ...
  const dataMap = new Map(visitorData.map(d => [d.visitorId, d]));
