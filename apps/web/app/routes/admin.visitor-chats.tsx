  ...
  if (allVisitors.length > 0) {
    ...
    const messageStats = await db
      .select({
        ...
        // In SQLite/D1, max() safely gets the associated row's content for bare columns in the select
        lastMessageContent: visitorMessages.content,
        lastMessageDate: sql<string>`max(${visitorMessages.createdAt})`
      })
    ...
    visitorsWithData = allVisitors.map(v => {
      ...
      return {
        ...
        lastActive: stats?.lastMessageDate || v.createdAt,
