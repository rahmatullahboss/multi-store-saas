...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (visitorIds.length > 0) {
    // Optimization: Use batched Drizzle inArray queries to resolve N+1 issue
    const [counts, latestMessages] = await Promise.all([
      db
        .select({
      ...
      db
        .select({
          ...
          createdAt: sql<string>`max(${visitorMessages.createdAt})`,
        })
        .from(visitorMessages)
        .where(inArray(visitorMessages.visitorId, visitorIds))
        .groupBy(visitorMessages.visitorId),
    ...
    const countMap = new Map(counts.map((c) => [c.visitorId, c.count]));
    const latestMap = new Map(latestMessages.map((m) => [m.visitorId, m]));
    ...
    visitorsWithData = allVisitors.map((v) => {
      ...
      return {
        ...
        lastActive: latest?.createdAt || v.createdAt,
