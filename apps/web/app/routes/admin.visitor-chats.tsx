...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Replaced N+1 Promise.all mapping with a single batched inArray() query using GROUP BY.
  // SQLite guarantees the unaggregated `content` column will correspond to the row matching max(createdAt).
  ...
  if (allVisitors.length > 0) {
    ...
    const aggregatedMessages = await db
      .select({
        ...
        lastActiveNum: sql<number>`max(${visitorMessages.createdAt})`,
    ...
    const messageStatsMap = new Map(
      aggregatedMessages.map((stat) => [stat.visitorId, stat])
    );

    visitorsWithData = visitorsWithData.map((v) => {
      const stat = messageStatsMap.get(v.id);
      return {
        ...v,
        lastMessage: stat ? stat.lastMessage : 'No messages',
        lastActive: stat && stat.lastActiveNum ? new Date(stat.lastActiveNum) : v.createdAt,
