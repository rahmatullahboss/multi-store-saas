...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Fix N+1 query by grouping
  // SQLite uniquely returns bare columns from the row containing the `max()` aggregate.
  let visitorsWithData: Array<typeof allVisitors[0] & { lastMessage: string, lastActive: Date | null, messageCount: number }> = [];
  ...
  if (allVisitors.length > 0) {
    ...
    const aggregatedData = await db
      .select({
        ...
        // Since `createdAt` uses `mode: 'timestamp'`, `max` returns the timestamp numeric value
        lastActiveVal: max(visitorMessages.createdAt),
    ...
    const aggMap = new Map(aggregatedData.map(d => [d.visitorId, d]));

    visitorsWithData = allVisitors.map(v => {
      const data = aggMap.get(v.id);
      return {
        ...v,
        lastMessage: data?.lastMessage || 'No messages',
        lastActive: data?.lastActiveVal ? new Date((data.lastActiveVal as unknown as number) * 1000) : v.createdAt,
