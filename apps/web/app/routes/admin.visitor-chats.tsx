...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (visitorIds.length > 0) {
    aggregatedMessages = await db
      .select({
        ...
        lastMessage: visitorMessages.content, // SQLite extracts the value from the row matching the MAX aggregate
  ...
  const visitorsWithData = allVisitors.map(v => {
    const stats = aggregatedMessages.find(m => m.visitorId === v.id);
    return {
        ...v,
