...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Optimized N+1 queries into a single query using grouping and inArray
  ...
  if (allVisitors.length > 0) {
    ...
    const aggregatedData = await db
      .select({
        ...
        latestMessageContent: visitorMessages.content,
        latestMessageCreatedAt: sql<number>`max(${visitorMessages.createdAt})`,
      })
    ...
    visitorsWithData = visitorsWithData.map((v) => {
      ...
      return {
        ...v,
        lastMessage: agg?.latestMessageContent || 'No messages',
        lastActive: agg?.latestMessageCreatedAt ? new Date(agg.latestMessageCreatedAt) : v.createdAt,
