...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Fix N+1 query problem using inArray with GROUP BY and MAX()
  // SQLite guarantees that the bare column (content) comes from the row with the max(createdAt)
  ...
  if (visitorIds.length > 0) {
    ...
    messageStats.forEach(s => statsMap.set(s.visitorId, s));
  }
