...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (visitorIds.length > 0) {
    // ⚡ Bolt: Resolve N+1 query problem by batching relation fetches using inArray and groupBy
    aggregatedData = await db
      .select({
        visitorId: visitorMessages.visitorId,
        messageCount: sql<number>`count(*)`.mapWith(Number),
        lastMessage: visitorMessages.content, // SQLite returns value from row matching MAX() when grouping
