...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: Use a single inArray query with grouping and max() aggregate
  // SQLite natively returns the corresponding bare column values (e.g. content) from the row with the max value
  const visitorIds = allVisitors.map(v => v.id);
  
  let messagesData: { visitorId: number | null; count: number; maxDate: Date | number | null; content: string }[] = [];
  ...
  if (visitorIds.length > 0) {
    messagesData = await db
      .select({
        ...
        maxDate: max(visitorMessages.createdAt),
  ...
  const messagesMap = new Map(messagesData.map(m => [m.visitorId, m]));

  const visitorsWithData = allVisitors.map(v => {
    const data = messagesMap.get(v.id);
    return {
        ...v,
        lastMessage: data?.content || 'No messages',
        lastActive: data?.maxDate ? new Date(data.maxDate) : v.createdAt,
