...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const allVisitors = await db
    ...
    .limit(50); // Warning: Do not increase past 50. D1 db.batch has a hard limit of 100 statements. 50 visitors * 2 queries = 100 statements max.

  // Fetch messages for these visitors to get counts and preview
  // ⚡ Bolt Optimization: Use a single HTTP call to D1 with db.batch instead of N+1 sequential requests
  // Cloudflare D1 HTTP overhead is high, so grouping the Drizzle queries into a batch prevents severe latency
  
  const visitorsWithData: Array<typeof allVisitors[0] & { lastMessage: string, lastActive: Date | number | null, messageCount: number }> = [];

  if (allVisitors.length > 0) {
    // Generate queries for batch execution
    const batchQueries = allVisitors.flatMap((v) => [
      db.select().from(visitorMessages).where(eq(visitorMessages.visitorId, v.id)).orderBy(desc(visitorMessages.createdAt)).limit(1),
      db.select({ count: sql<number>`count(*)` }).from(visitorMessages).where(eq(visitorMessages.visitorId, v.id))
    ]);

    // Perform one batched DB HTTP request
    const batchResults = await db.batch(batchQueries as any);

    // Rehydrate results mapping back to respective visitor
    allVisitors.forEach((v, index) => {
      const messages = batchResults[index * 2] as typeof visitorMessages.$inferSelect[];
      const countResult = batchResults[(index * 2) + 1] as { count: number }[];

      visitorsWithData.push({
        ...v,
...
type LoaderData = {
  visitors: Array<typeof visitors.$inferSelect & { lastMessage: string, lastActive: Date | number | null, messageCount: number }>;
  selectedMessages: typeof visitorMessages.$inferSelect[];
  selectedVisitor: (typeof visitors.$inferSelect & { lastMessage: string, lastActive: Date | number | null, messageCount: number }) | null;
};
