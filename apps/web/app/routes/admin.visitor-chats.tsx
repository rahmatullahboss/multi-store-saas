...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  let visitorsWithData: Array<typeof allVisitors[0] & {
    lastMessage: string;
  ...
  if (visitorIds.length > 0) {
    // Optimization: Replace N+1 queries with two batch queries using grouping
    const [latestMessages, messageCounts] = await Promise.all([
      db
        .select({
          visitorId: visitorMessages.visitorId,
          content: visitorMessages.content,
          maxDate: sql<number>`max(${visitorMessages.createdAt})`
        })
    ...
    const latestMsgMap = new Map(latestMessages.map(m => [m.visitorId, m]));
    const countMap = new Map(messageCounts.map(m => [m.visitorId, m.count]));

    visitorsWithData = allVisitors.map(v => {
      const latestMsg = latestMsgMap.get(v.id);
      return {
        ...v,
        lastMessage: latestMsg?.content || 'No messages',
        lastActive: latestMsg ? new Date(latestMsg.maxDate) : v.createdAt,
        messageCount: countMap.get(v.id) || 0
      };
    });
  } else {
    visitorsWithData = [];
