...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (visitorIds.length > 0) {
    // ⚡ Bolt: Use a single query with grouping and max() for SQLite bare column value retrieval
    // This resolves the massive N+1 connection overhead
    aggregatedData = await db
      .select({
        visitorId: visitorMessages.visitorId,
        count: sql<number>`count(*)`.mapWith(Number),
        latestContent: visitorMessages.content,
  ...
  const visitorsWithData = allVisitors.map((v) => {
    const data = aggregatedData.find((d) => d.visitorId === v.id);
    return {
      ...v,
      lastMessage: data?.latestContent || 'No messages',
      lastActive: data?.maxCreatedAt ? new Date(data.maxCreatedAt) : v.createdAt,
