...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimized: Single query using GROUP BY to solve N+1 connection latency
  const visitorIds = allVisitors.map(v => v.id);
  
  const messagesAgg = visitorIds.length > 0 ? await db
    .select({
  ...
  const messagesMap = new Map(messagesAgg.map(agg => [agg.visitorId, agg]));

  const visitorsWithData = allVisitors.map(v => {
    const agg = messagesMap.get(v.id);
    return {
        ...v,
        lastMessage: agg?.lastMessage || 'No messages',
        lastActive: agg?.lastActive ? new Date(agg.lastActive) : v.createdAt,
