...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Replaced N+1 query loops with a single batch inArray fetch
  ...
  if (allVisitors.length > 0) {
    ...
    const latestMessagesMap = new Map(latestMessages.map(m => [
      m.visitorId,
      { content: m.content, createdAt: new Date(m.createdAt) }
    ]));

    visitorsWithData = allVisitors.map(v => {
      const count = countsMap.get(v.id) || 0;
      const latest = latestMessagesMap.get(v.id);

      return {
        ...v,
        lastMessage: latest ? latest.content : 'No messages',
        lastActive: latest ? latest.createdAt : v.createdAt,
        messageCount: count
      };
    });
  }
