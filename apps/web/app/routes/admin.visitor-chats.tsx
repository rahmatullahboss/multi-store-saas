...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Resolved N+1 query bottleneck using grouped counts and batched requests.
  ...
  if (visitorIds.length > 0) {
    ...
    countResults.forEach((r) => {
      countsMap[r.visitorId] = r.count;
    });
  }

  // To prevent unbounded memory fetches (OOM), we retain bounded latest message fetches
  // but execute them concurrently rather than compounding an N+1 count bottleneck.
