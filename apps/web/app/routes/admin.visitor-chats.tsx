...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Fixed N+1 queries by using a single grouped query with inArray.
  // This significantly reduces the number of queries and speeds up the loader.
  ...
  const statsMap = new Map(messageStats.map(stat => [stat.visitorId, stat]));
  ...
  const visitorsWithData = allVisitors.map(v => {
    ...
    return {
      ...
      // Drizzle 'timestamp' mode stores epoch in seconds. Rehydrate to milliseconds for Date
      lastActive: stat?.lastActive ? new Date(stat.lastActive * 1000) : v.createdAt,
