...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Fetch messages using a single query to eliminate N+1 latency
  const visitorIds = allVisitors.map((v) => v.id);
  const messageStats = visitorIds.length > 0
    ? await db
        .select({
          visitorId: visitorMessages.visitorId,
          content: visitorMessages.content,
          createdAt: max(visitorMessages.createdAt),
  ...
  const visitorsWithData = allVisitors.map((v) => {
    ...
    return {
        ...
        lastActive: stats?.createdAt ? new Date(stats.createdAt as unknown as number) : v.createdAt,
