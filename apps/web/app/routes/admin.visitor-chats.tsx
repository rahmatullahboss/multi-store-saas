...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Optimized N+1 queries into a single grouped query.
  const visitorIds = allVisitors.map(v => v.id);
  const messagesData = visitorIds.length > 0
    ? await db.select({
        visitorId: visitorMessages.visitorId,
        lastMessage: visitorMessages.content,
        // In Drizzle with Cloudflare D1/SQLite, aggregate on timestamp returns a number (Unix timestamp in ms / seconds)
  ...
  const visitorsWithData = allVisitors.map((v) => {
    const data = messagesData.find(m => m.visitorId === v.id);
    return {
        ...v,
