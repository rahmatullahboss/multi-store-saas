...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  let messageStats: { visitorId: number, count: number, latestMessage: string | null, latestCreatedAt: string | null }[] = [];
  ...
  if (visitorIds.length > 0) {
    messageStats = await db
      .select({
        ...
        latestMessage: visitorMessages.content,
  ...
  const visitorsWithData = allVisitors.map(v => {
    ...
    return {
      ...
      lastActive: stats?.latestCreatedAt || v.createdAt,
