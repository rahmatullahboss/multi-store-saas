...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: ⚡ Bolt: Replaced N+1 Promise.all query with a single grouped inArray query.
  // This drastically reduces DB network roundtrips on Cloudflare D1.
  const visitorIds = allVisitors.map((v) => v.id);
  
  // We use Record type to maintain strong type safety and avoid any[] casts
  const messageDataMap: Record<number, { count: number; content: string | null; maxDate: any }> = {};

  if (visitorIds.length > 0) {
    const groupedMessages = await db.select({
      visitorId: visitorMessages.visitorId,
      count: sql<number>`count(*)`,
      content: visitorMessages.content, // SQLite natively returns the corresponding bare column for the max aggregate
      maxDate: sql<any>`max(${visitorMessages.createdAt})`
    })
  ...
  const visitorsWithData = allVisitors.map((v) => {
    ...
    return {
        ...
        lastActive: data?.maxDate || v.createdAt,
