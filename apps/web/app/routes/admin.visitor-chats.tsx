...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimized to use a single aggregated query instead of N+1 Promise.all mappings.
  ...
  const visitorsMap = new Map(aggregatedMessages.map(msg => [
    msg.visitorId,
    {
      count: msg.count,
      lastActive: msg.lastActive ? new Date(msg.lastActive) : null,
      lastMessage: msg.lastMessage
    }
  ]));

  const visitorsWithData = allVisitors.map((v) => {
    const stats = visitorsMap.get(v.id);
    return {
        ...v,
