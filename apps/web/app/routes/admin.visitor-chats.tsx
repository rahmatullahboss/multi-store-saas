...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  let visitorStats: { visitorId: number; count: number; lastActiveTs: number | null; lastMessage: string }[] = [];
  if (visitorIds.length > 0) {
    visitorStats = await db
      .select({
        visitorId: visitorMessages.visitorId,
        count: sql<number>`cast(count(*) as integer)`,
        lastActiveTs: sql<number>`max(${visitorMessages.createdAt})`,
  ...
  const statsMap = new Map(visitorStats.map(stat => [stat.visitorId, stat]));
  ...
  const visitorsWithData = allVisitors.map(v => {
    ...
    return {
      ...
      lastActive: stats?.lastActiveTs ? new Date(stats.lastActiveTs) : v.createdAt,
