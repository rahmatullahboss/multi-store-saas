...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Fixed N+1 queries by replacing Promise.all mapping with a single grouped query.
  // We extract IDs and use inArray() with GROUP BY and aggregate max() to fetch counts
  // and the latest message concurrently. This reduces network roundtrips to Cloudflare D1
  // from O(2N) to O(1), improving load times significantly.
  const visitorIds = allVisitors.map(v => v.id);
  
  const statsMap: Record<number, { count: number; lastMessage: string | null; lastActive: Date | null }> = {};
  ...
  if (visitorIds.length > 0) {
    ...
    for (const stat of statsResult) {
      if (stat.visitorId) {
        statsMap[stat.visitorId] = {
          count: stat.count,
          lastMessage: stat.lastMessage,
          lastActive: stat.lastActive ? new Date(stat.lastActive) : null
        };
      }
    }
  }
  ...
  const visitorsWithData = allVisitors.map(v => {
    const stat = statsMap[v.id];
    return {
      ...v,
      lastMessage: stat?.lastMessage || 'No messages',
      lastActive: stat?.lastActive || v.createdAt,
