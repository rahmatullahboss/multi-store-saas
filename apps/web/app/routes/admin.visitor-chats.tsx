...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Fetch messages for these visitors to get counts and preview
  // Optimized: Replaced N+1 Promise.all query with a single bulk query using inArray and groupBy
  // Uses SQLite's bare column aggregation to fetch the lastMessage for the max(createdAt) row.
  ...
  if (visitorIds.length > 0) {
    statsResult = await db
      .select({
        ...
        lastMessage: visitorMessages.content, // SQLite specific bare column behavior
  ...
  const visitorsWithData = allVisitors.map(v => {
    ...
    return {
        ...
        // Drizzle `max` with timestamp mode returns seconds as a number instead of parsed Date
        lastActive: stats?.lastActive ? new Date(stats.lastActive * 1000) : v.createdAt,
