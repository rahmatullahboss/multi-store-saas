...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: Use a single query with grouping for Drizzle/SQLite.
  const visitorIds = allVisitors.map(v => v.id);
  
  let counts: Record<number, number> = {};
  let latestMessages: Record<number, { content: string, createdAt: string }> = {};

  if (visitorIds.length > 0) {
    const batchedCounts = await db
      .select({
    ...
    for (const row of batchedCounts) {
      if (row.visitorId) counts[row.visitorId] = row.count;
    }

    const batchedLatest = await db
      .select({
        visitorId: visitorMessages.visitorId,
    ...
    for (const row of batchedLatest) {
      if (row.visitorId) {
        latestMessages[row.visitorId] = {
          content: row.content,
          createdAt: row.lastActive
        };
      }
    }
  }
  ...
  const visitorsWithData = allVisitors.map(v => ({
    ...v,
    lastMessage: latestMessages[v.id]?.content || 'No messages',
    lastActive: latestMessages[v.id]?.createdAt || v.createdAt,
    messageCount: counts[v.id] || 0
  }));
