...
import { desc, sql, eq, inArray, max, count } from 'drizzle-orm';
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt Optimization: Fix N+1 query issue for visitor messages
  // Using a single query with grouping to get message counts and latest message info.
  // SQLite natively returns the corresponding bare column values from the row containing the maximum value.
  ...
  if (visitorIds.length > 0) {
    const messageStats = await db
      .select({
        ...
        messageCount: count(visitorMessages.id),
    ...
    statsMap = new Map(messageStats.map((s) => [s.visitorId, s]));
  }
