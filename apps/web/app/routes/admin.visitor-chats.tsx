...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: Use a single query with grouping to prevent N+1 performance bottleneck.
  
  const visitorIds = allVisitors.map(v => v.id);

  let aggregatedMessages: { visitorId: number; lastMessage: string; lastActive: number; messageCount: number }[] = [];

  if (visitorIds.length > 0) {
    aggregatedMessages = await db
      .select({
        visitorId: visitorMessages.visitorId,
        lastMessage: visitorMessages.content,
        // Drizzle SQLite returns integer mode='timestamp' as raw ms from aggregates
  ...
  const messagesMap = new Map(aggregatedMessages.map(m => [m.visitorId, m]));

  const visitorsWithData = allVisitors.map(v => {
    const stats = messagesMap.get(v.id);
    return {
      ...v,
      lastMessage: stats?.lastMessage || 'No messages',
      // Re-hydrate the ms timestamp to a Date object, fallback to v.createdAt
      lastActive: stats?.lastActive ? new Date(stats.lastActive) : v.createdAt,
