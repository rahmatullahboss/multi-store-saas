...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Optimization: Resolve N+1 query using inArray with groupBy and SQLite max() behavior
  ...
  if (visitorIds.length > 0) {
    const messageStats = await db
      .select({
        ...
        lastContent: visitorMessages.content,
      })
  ...
  const visitorsWithData = allVisitors.map((v) => {
    ...
    return {
      ...v,
      lastMessage: stats?.lastContent || 'No messages',
