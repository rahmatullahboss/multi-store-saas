...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // ⚡ Bolt: Replaced N+1 query loops with a single bulk query using inArray and a correlated subquery
  let visitorsWithData = allVisitors.map(v => ({ ...v, lastMessage: 'No messages', lastActive: v.createdAt, messageCount: 0 }));
  ...
  if (allVisitors.length > 0) {
    ...
    const aggregatedStats = await db
      .select({
        ...
        maxCreatedAt: sql<number>`max(${visitorMessages.createdAt})`.mapWith(Number),
        latestMessage: sql<string>`(
          SELECT content
          FROM visitor_messages v2
          WHERE v2.visitor_id = visitor_messages.visitor_id
    ...
    const statsMap = new Map(aggregatedStats.map(s => [s.visitorId, s]));

    visitorsWithData = allVisitors.map(v => {
      const stats = statsMap.get(v.id);
      return {
        ...v,
        lastMessage: stats?.latestMessage || 'No messages',
        // Drizzle max() on timestamp integer returns numeric epoch, rehydrate to Date
        lastActive: stats?.maxCreatedAt ? new Date(stats.maxCreatedAt) : v.createdAt,
