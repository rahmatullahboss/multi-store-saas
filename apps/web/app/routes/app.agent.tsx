...
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
  ...
  // Daily Stats (Last 7 Days) setup
  const sevenDaysAgo = new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

  // Calculate Metrics - Execute queries concurrently for better performance
  // ⚡ Bolt: Optimizing DB latency by parallelizing independent queries
  const [[convCount], [leadsCount], [ordersCount], dailyDataRaw] = await Promise.all([
    db
      .select({ count: count() })
    ...
    db
      ...
      .innerJoin(
        ...
        eq(schema.leadsData.conversationId, schema.aiConversations.id)
      )
      .where(eq(schema.aiConversations.agentId, agent?.id || 0)),

    db.select({ count: count() }).from(schema.orders).where(eq(schema.orders.storeId, storeId)),
  ...
  if (isLocked) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <p className="text-gray-500 max-w-md mb-6">{t('premiumFeatureDesc')}</p>
        <Link
          to="/app/billing"
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`px-3 py-1 rounded-full text-sm font-medium ${agent?.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}
        >
              ...
              <LazyAreaChart
                data={
                  (stats?.daily && stats.daily.length > 0
                    ? stats.daily
                    : [{ date: 'Today', count: 0 }]) as Record<string, unknown>[]
                }
                height={300}
                dataKey="count"
                xAxisKey="date"
