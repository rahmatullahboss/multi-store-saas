...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (intent === 'create-store') {
        ...
        if (!name || !contact_name || !contact_number || !address || !city_id || !zone_id || !area_id) {
...
export default function CourierSettingsPage() {
  const { settings, cities } = useLoaderData<typeof loader>() as { settings: CourierSettings; cities: { city_id: number; city_name: string }[] };
            ...
            {providers.map((provider) => (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className={`font-medium ${selectedProvider === provider.id ? 'text-emerald-900' : 'text-gray-900'}`}>
                      {provider.name}
                    </h3>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 mt-1">
                  You can now create a new Pathao store directly from here. Useful for Sandbox testing!
                </p>
                <button
                  type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Leave empty for default. For Sandbox, use: <code>https://courier-api-sandbox.pathao.com/aladdin/api/v1</code>
                      </p>
                    </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex flex-col justify-center">
                           <span className="text-[10px] text-gray-500 px-2 leading-tight">
                              Required for<br/>creating orders
                           </span>
                        </div>
                      </div>
                      ...
                      {actionData && 'stores' in actionData && Array.isArray(actionData.stores) && (
                        <div className="mt-3 p-3 bg-emerald-50 rounded-lg border border-emerald-100 animate-in fade-in slide-in-from-top-2 duration-300">
                           <div className="flex justify-between items-center mb-2">
                            <p className="text-xs font-semibold text-emerald-800">Available Stores found in your account:</p>
                          ...
                          <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                            {actionData.stores?.map((store: PathaoStore) => (
                              <div key={store.store_id} className="flex justify-between items-start text-xs p-2 bg-white rounded border border-emerald-100 shadow-sm hover:border-emerald-300 transition-colors">
                                <div>
                                  <p className="font-medium text-emerald-900">{store.store_name}</p>
                                  <p className="text-gray-500 line-clamp-1" title={store.store_address}>{store.store_address}</p>
                                </div>
                                <div className="flex flex-col items-end gap-1">
                                  ...
                                  <button 
                                    type="button"
                                    onClick={() => {
                                      const input = document.querySelector('input[name="defaultStoreId"]') as HTMLInputElement;
...
function CreateStoreModal({ cities }: { cities: { city_id: number; city_name: string }[] }) {
    ...
    return (
        <dialog id="create-store-modal" className="modal rounded-xl shadow-2xl p-0 backdrop:bg-black/50" ref={modalRef}>
                    ...
                    <fetcher.Form method="post" id="create-store-form" className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="contact_name" required placeholder="John Doe" className="w-full px-3 py-2 border rounded-lg focus:ring-emerald-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                                <input type="text" name="contact_number" required placeholder="017XXXXXXXX" className="w-full px-3 py-2 border rounded-lg focus:ring-emerald-500" />
                            </div>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Address</label>
                            <textarea name="address" required placeholder="Full street address..." className="w-full px-3 py-2 border rounded-lg focus:ring-emerald-500" rows={2}></textarea>
                        </div>
                        ...
                        <div className="pt-4">
                            <button type="submit" disabled={fetcher.state === 'submitting'} className="w-full bg-emerald-600 text-white py-2 rounded-lg font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50">
