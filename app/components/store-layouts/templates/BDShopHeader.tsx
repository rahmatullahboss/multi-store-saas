...
import { 
  ...
  Zap, ChevronDown, ChevronRight, Home as HomeIcon, Grid3X3
} from 'lucide-react';
import { BDSHOP_THEME } from '~/components/store-templates/BDShopTheme';
...
export function BDShopHeader({ storeName, logo, isPreview, categories = [], currentCategory }: BDShopHeaderProps) {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center h-16 gap-4">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0">
            {logo ? (
              ...
              <div className="flex items-center gap-1">
                <div className="h-10 w-10 rounded flex items-center justify-center" style={{ backgroundColor: BDSHOP_THEME.primary }}>
                  <ShoppingBag className="w-6 h-6 text-white" />
                </div>
                <span className="font-bold text-xl hidden sm:block" style={{ color: BDSHOP_THEME.primary }}>{storeName}</span>
              </div>
            )}
          </Link>
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative flex">
              ...
              />
              <button className="absolute right-0 top-0 h-full px-4 rounded-r-lg text-white" style={{ backgroundColor: BDSHOP_THEME.primary }}>
                <Search size={20} />
              </button>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link to="/cart" className="relative p-2 hover:bg-gray-100 rounded-lg transition">
              ...
              <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: BDSHOP_THEME.accent }}>
                {count}
              </span>
            </Link>
          </div>
      ...
      {categoryDrawerOpen && (
        <>
          <div className="fixed inset-0 bg-black/50 z-[60]" onClick={() => setCategoryDrawerOpen(false)} />
          <div 
            className="fixed left-0 top-0 bottom-0 w-72 bg-white z-[70] shadow-xl overflow-y-auto transform transition-transform duration-300 ease-in-out"
          >
            <div className="sticky top-0 bg-white p-4 border-b flex items-center justify-between">
              <h2 className="font-bold text-lg text-gray-800">Shop by Category</h2>
              <button onClick={() => setCategoryDrawerOpen(false)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500">
                <X className="w-5 h-5" />
              </button>
            </div>
