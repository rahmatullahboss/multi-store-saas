...
export function MarketingHeader({ showBackToHome = false }: { showBackToHome?: boolean }) {
  ...
  return (
    <header className="fixed top-2 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-5xl">
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl px-4 py-0 shadow-lg">
            ...
            {!showBackToHome ? (
              <>
                {!isActive('/pricing') && (
                  <Link 
                    to="/pricing" 
                    className={`hidden md:block font-medium text-sm px-3 py-1.5 transition ${
                ...
                <Link 
                  to="/auth/login" 
                  className="hidden sm:block text-white/60 hover:text-white font-medium text-sm px-4 py-1.5 transition"
                >
                  {t('login')}
                </Link>
                <MagneticButton>
                  <Link 
                    to="/auth/register" 
                    className="hidden sm:inline-block px-5 py-2 bg-gradient-to-r from-[#006A4E] to-[#00875F] hover:from-[#005740] hover:to-[#006A4E] text-white font-semibold rounded-xl text-sm transition shadow-lg shadow-[#006A4E]/25"
                  >
                    {t('register')}
                  </Link>
                </MagneticButton>
