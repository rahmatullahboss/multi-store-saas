...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const activeStores = await db
    ...
    .from(stores)
    .where(or(
      eq(stores.aiAgentRequestStatus, 'approved'),
...
export default function AdminAiRequests() {
  ...
  const getPlanBadgeColor = (plan: string) => {
    switch (plan) {
      case 'starter': return 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30';
  ...
  const filteredUsageStats = activeUsageStats.filter(store => 
              ...
              <table className="w-full">
                <thead className="bg-slate-800/50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Requested At</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Payment Info</th>
                  ...
                  {pendingRequests.map((request) => {
                    ...
                    return (
                      <tr key={request.id} className="hover:bg-slate-800/30 transition">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 text-xs font-medium rounded-full border ${getPlanBadgeColor(request.planType || 'free')}`}>
