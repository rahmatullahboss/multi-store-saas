...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const pageId = pageIdParam ? parseInt(pageIdParam) : null;
  ...
  let landingConfig = parseLandingConfig(store.landingConfig as string | null) || defaultLandingConfig;
  let pageTitle = 'Store Landing Page';

  // If editing a specific campaign page
  if (pageId) {
    const page = await db
      .select()
      .from(savedLandingConfigs)
      .where(and(eq(savedLandingConfigs.storeId, storeId), eq(savedLandingConfigs.id, pageId)))
      .limit(1)
      .get();

    if (page) {
      landingConfig = parseLandingConfig(page.landingConfig as string | null) || defaultLandingConfig;
      pageTitle = page.name || 'Campaign Page';
    }
  }
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  const pageIdRaw = formData.get('pageId') as string;
  const pageId = pageIdRaw ? parseInt(pageIdRaw) : null;

  // Helper to update config based on target (Store vs Page)
  async function updateConfig(newConfig: LandingConfig) {
    if (pageId) {
      // Update Saved Page
      await db
        ...
        .set({
          ...
          // Also update active state if implicit? No, keep it simple.
        })
        .where(and(eq(savedLandingConfigs.id, pageId as number), eq(savedLandingConfigs.storeId, storeId)));
    } else {
      // Update Store Landing
      await db
        .update(stores)
        .set({
          landingConfig: JSON.stringify(newConfig),
          updatedAt: new Date(),
          // If saving store landing, ensures mode is set? Maybe not force it here.
        })
        .where(eq(stores.id, storeId));
    }
  }
  ...
  async function getCurrentConfig() {
    if (pageId) {
      const page = await db
        .select()
        .from(savedLandingConfigs)
        .where(and(eq(savedLandingConfigs.storeId, storeId), eq(savedLandingConfigs.id, pageId as number)))
        .get();
      return parseLandingConfig(page?.landingConfig as string | null) || defaultLandingConfig;
    } else {
      const storeResult = await db.select().from(stores).where(eq(stores.id, storeId)).limit(1);
      return parseLandingConfig(storeResult[0]?.landingConfig as string | null) || defaultLandingConfig;
    }
  }
...
export default function LandingBuilderPage() {
  ...
  useEffect(() => {
    ...
    const autoSaveInterval = setInterval(() => {
      ...
      if (pageId) formData.append('pageId', pageId.toString());
            ...
            <div className="flex items-center gap-2 sm:gap-4 min-w-0">
              ...
              <div className="min-w-0 hidden sm:block">
                ...
                <p className="text-xs sm:text-sm text-gray-500 truncate">
                  {pageTitle || store.name} • {selectedTemplate?.emoji} {language === 'bn' ? selectedTemplate?.name : selectedTemplate?.nameEn}
                </p>
              </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
              ...
              >
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-2.5 sm:px-4 py-1.5 sm:py-2 font-medium rounded-lg transition text-sm ${hasChanges
                ...
                {autoSaveStatus !== 'idle' && (
                  <span className={`hidden sm:inline text-xs px-2 py-1 rounded-full ${autoSaveStatus === 'saving' ? 'bg-blue-100 text-blue-600' :
          ...
          <div className="flex gap-1.5 sm:gap-2 mb-4 overflow-x-auto pb-2 -mx-2 px-2">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1 sm:gap-2 px-2.5 sm:px-4 py-2 rounded-lg font-medium transition whitespace-nowrap text-xs sm:text-sm ${activeTab === tab.id
                    ...
                    <div className="flex flex-wrap gap-3 mb-4">
                      {[
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-lg border-2 transition-all ${primaryColor === color.value
                    ...
                    <div className="flex flex-wrap gap-3 mb-4">
                      {[
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-lg border-2 transition-all ${accentColor === color.value
                            ? 'ring-2 ring-offset-2 ring-gray-400 border-gray-400'
                            : 'border-gray-200 hover:scale-110'
                            }`}
                          style={{ backgroundColor: color.value }}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 flex items-center justify-between gap-3 z-50 shadow-lg">
        ...
        >
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className={`w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 font-medium rounded-lg disabled:opacity-50 ${hasChanges ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
              }`}
          >
