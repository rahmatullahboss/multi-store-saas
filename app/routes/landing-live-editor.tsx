        ...
        `}>
          ...
          >
            <div className="space-y-4">
              ...
              {/* Custom HTML Section Import */}
              <div className="border-t border-gray-200 pt-4 mt-4">
                <label className="block text-xs font-medium text-gray-700 mb-2">
                  {language === 'bn' ? '🎨 কাস্টম HTML ডিজাইন' : '🎨 Custom HTML Design'}
                </label>
                <p className="text-xs text-gray-500 mb-2">
                  {language === 'bn' 
                    ? 'পুরো HTML ডিজাইন paste করুন (CSS সহ)'
                    : 'Paste your complete HTML design (with CSS)'}
                </p>
                ...
                <textarea
                  placeholder={language === 'bn' 
                    ...
                    : `<!-- Your HTML here -->\n<div style="...">\n  ...\n</div>`}
                  rows={6}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs font-mono mb-2"
                  id="custom-html-input"
                />
                <button
                  type="button"
                  onClick={() => {
                    const textarea = document.getElementById('custom-html-input') as HTMLTextAreaElement;
                    const html = textarea?.value?.trim();
                    if (html) {
                      const newSection = {
                        id: `custom-${Date.now()}`,
                        name: `Custom Design ${customSections.length + 1}`,
                        html: html,
                      };
                  ...
                  className="w-full px-3 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-medium rounded-lg hover:from-purple-600 hover:to-pink-600 transition flex items-center justify-center gap-2"
                >
                  <Plus className="w-4 h-4" />
                  {language === 'bn' ? 'HTML সেকশন যোগ করুন' : 'Add HTML Section'}
                </button>
              </div>
            </div>
          </AccordionSection>
