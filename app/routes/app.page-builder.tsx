 ...
 * Page Builder Route
 * 
 * This route manages landing pages and redirects to the 
 * separate page-builder worker for editing.
 * 
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  // Redirect to page-builder worker
  const builderUrl = env.PAGE_BUILDER_URL || 'https://builder.ozzyl.com';
  return redirect(`${builderUrl}/edit/${newPage.id}`);
}
...
export default function PageBuilderRoute() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('pageBuilderTitle') || 'Page Builder'}</h1>
          <p className="text-gray-500">{t('pageBuilderDesc') || 'Create stunning landing pages'}</p>
        </div>
        <button 
      ...
      {isCreating && (
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-emerald-100 animate-in fade-in slide-in-from-top-4 duration-300">
          <h3 className="text-lg font-bold mb-6 text-gray-900">{t('newPageDetails') || 'New Page Details'}</h3>
          <Form method="post" className="space-y-6 max-w-xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('internalPageName') || 'Page Name'}</label>
                <input 
                  autoFocus
                  name="name" 
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('urlSlug') || 'URL Slug'}</label>
                <input 
                  name="slug" 
                  placeholder="summer-offer" 
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition"
                />
                <p className="mt-1.5 text-[10px] text-gray-400 uppercase font-bold tracking-wider">{t('urlSlugHint') || 'Leave empty for auto-generate'}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 pt-2">
              ...
              >
                {navigation.state === 'submitting' ? t('creating') || 'Creating...' : t('startBuilding') || 'Create & Edit'}
              </button>
              <button 
                type="button"
          ...
          <div className="col-span-full py-20 bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center gap-4">
             <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center">
              ...
              <div className="text-center">
                <p className="text-gray-900 font-bold">{t('noPagesCreated') || 'No pages yet'}</p>
                <p className="text-gray-500 text-sm">{t('noPagesDesc') || 'Create your first landing page'}</p>
              </div>
          </div>
        ) : (
          pages.map((page) => (
            <div key={page.id} className="group bg-white rounded-3xl p-6 shadow-sm border border-gray-100 hover:border-emerald-200 transition-all hover:shadow-xl hover:shadow-emerald-500/5 hover:-translate-y-1">
               <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-gray-50 rounded-full text-[10px] font-bold text-gray-500 uppercase">
                     ...
                     {page.isPublished ? t('live') || 'Live' : t('draft') || 'Draft'}
                  </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-3">
                  ...
                  {page.isPublished ? (
                    ...
                    >
                       {t('view') || 'View'} <ExternalLink size={12} />
                    </a>
                  ) : (
