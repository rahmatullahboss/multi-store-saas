...
export function OrderFormFields({
  ...
  if (state.orderSuccess) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <h3 className="text-2xl font-bold text-green-600 mb-2">{thankYouHeadline}</h3>
  ...
  return (
    <>
      ...
      <fetcher.Form method="post" action="/api/create-order" className="space-y-4">
        {/* Hidden inputs */}
        <input type="hidden" name="store_id" value={storeId || ''} />
        <input
          type="hidden"
          name="product_id"
          value={cartItems && cartItems.length > 0 ? '' : productId || ''}
        />
        ...
        <input
          type="hidden"
          name="division"
          value={calculatedShippingZone === 'dhaka' ? 'dhaka' : 'outside_dhaka'}
        />
        ...
        <div>
          <div className="relative">
            ...
            {state.phone && (
              ...
              >
                {validateBDPhone(state.phone)
                  ? '✓'
                  : `${state.phone.replace(/[\s-]/g, '').length}/11`}
              </span>
            )}
          </div>
          {state.phoneError && <p className="text-red-500 text-xs mt-1">{state.phoneError}</p>}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <span
                ...
                style={{
                  backgroundColor: state.isInsideDhaka
                    ? 'rgba(255,255,255,0.2)'
                    : `${primaryColor}20`,
                  color: state.isInsideDhaka ? '#FFFFFF' : primaryColor,
                }}
              >
                {formatPrice(insideDhakaCharge)}
              </span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              >
                {formatPrice(outsideDhakaCharge)}
              </span>
            </button>
          </div>
        )}
        ...
        >
          ...
          <div className="space-y-2">
            ...
            {comboSummary && comboSummary.savings > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-600 font-semibold">
                  -{formatPrice(comboSummary.savings)}
                </span>
              </div>
            )}
            <div className="flex justify-between text-sm">
            ...
            {props.showFreeShippingProgress !== false &&
              props.freeShippingThreshold &&
              subtotal >= props.freeShippingThreshold && (
                <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between pt-2 border-t" style={{ borderColor: cardBorder }}>
              ...
              <span className="font-bold text-xl" style={{ color: primaryColor }}>
                {formatPrice(
                  props.showFreeShippingProgress !== false &&
                    props.freeShippingThreshold &&
                    subtotal >= props.freeShippingThreshold
                    ? (comboSummary?.discountedSubtotal ?? subtotal)
                    : comboSummary?.discountedSubtotal
                      ? comboSummary.discountedSubtotal + deliveryCharge
                      : total
                )}
              </span>
            </div>
