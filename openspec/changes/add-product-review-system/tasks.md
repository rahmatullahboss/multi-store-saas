...
- [x] 1.1 Create Drizzle schema for reviews table in `packages/database/src/schema.ts`
- [x] 1.2 Add composite index on (store_id, product_id)
- [x] 1.3 Generate and run migration with `npm run db:migrate:local`
- [x] 1.4 Verify table created with Drizzle Studio

## 2. Type Definitions

- [x] 2.1 Add Review type in shared types
- [x] 2.2 Add ReviewStatus type ('pending' | 'approved' | 'rejected')
- [x] 2.3 Add API response types for reviews

## 3. Review Submission (Storefront)

- [ ] 3.1 Enhance POST action for `/api/reviews` route - add verified purchase check
- [ ] 3.2 Validate customer purchased product (check order status = delivered)
- [x] 3.3 Check rate limit (no existing review per customer per product)
- [x] 3.4 Validate rating (1-5) and comment (10-1000 chars)
- [x] 3.5 Insert review with status 'pending'
- [x] 3.6 Add loader to fetch reviews for product page
- [x] 3.7 Update product detail page to load and display reviews

## 4. Review Display (Storefront)

- [x] 4.1 Create StarRating component in `app/components/store/`
- [x] 4.2 Create ProductReviews component
- [ ] 4.3 Add pagination (10 per page)
- [ ] 4.4 Add sorting (Recent, Highest, Lowest)
- [x] 4.5 Add review summary (avg rating + count) to product detail page
...
- [x] 6.4 Add approve action (update status to 'approved') - existing API
- [x] 6.5 Add reject action (update status to 'rejected') - existing API
