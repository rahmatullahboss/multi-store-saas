...
## 2026-03-20 - [Fix N+1 query problems using aggregates and subqueries in Drizzle]
**Learning:** Looping over records to fetch aggregates (like `count`) and latest records (like the newest message) leads to severe N+1 latency issues on Cloudflare D1.
**Action:** Always refactor sequential queries inside loops into a single batch query by extracting an array of IDs and using `inArray`. Use Drizzle's `.groupBy` along with the `sql` helper to calculate aggregates (e.g. `count`) and use correlated subqueries inside the `sql` helper to get the latest associated record.
