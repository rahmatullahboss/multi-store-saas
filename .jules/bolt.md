...
## 2026-05-09 - [Parallelize Sequential Pagination Database Queries]
**Learning:** When retrieving paginated rows and the total row count separately using Drizzle ORM, running them sequentially doubles the latency profile of the data fetch.
**Action:** Always identify independent database fetches like pagination rows and total count, and execute them concurrently using `Promise.all()` to mitigate DB round-trip latency.
