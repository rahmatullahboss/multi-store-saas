...
## 2026-03-24 - [Parallelize DB queries in Agent server]
**Learning:** Sequential database queries that don't depend on each other cause unnecessary latency on Cloudflare D1. In `agent.server.ts`, fetching agent config, conversation, and chat history sequentially compounded the response time.
**Action:** Always group independent Drizzle queries using `Promise.all` to execute them concurrently, reducing total DB time from sum-latency to max-latency.
