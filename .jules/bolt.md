...
## 2026-03-24 - [Optimize N+1 DB Queries using Drizzle inArray]
**Learning:** Using `Promise.all` alongside `.map()` to iterate over records (like grabbing chat counts for `visitors`) caused massive N+1 database connection/network latency over Cloudflare D1.
**Action:** Extract parent IDs into an array, perform an `inArray()` lookup combined with `GROUP BY` and an aggregate function `sql<string>\`max(...)\``, and structure the data into an in-memory dictionary.
