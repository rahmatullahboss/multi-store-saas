...
## 2026-03-24 - [Solve N+1 queries when fetching both aggregates and latest records]
**Learning:** Fetching the latest record and an aggregate count for a list of parent entities often leads to N+1 queries (e.g. iterating over 50 parents and doing a `limit(1)` + `count(*)` per parent). While generic SQL might require complex subqueries or window functions (like `DISTINCT ON` or `ROW_NUMBER()`), SQLite uniquely allows selecting non-aggregate columns alongside `max()` in a `GROUP BY` query. This feature returns the exact row containing the maximum value.
**Action:** When you need to resolve an N+1 fetching the latest related item and a count across a list of IDs, use two batched queries with `inArray` and `groupBy`: one for `sql<number>\`count(*)\`` and another using `sql<string>\`max(${column})\`` alongside the non-aggregate columns you need.
