...
## 2024-05-18 - [Centralize Intl Object Caching]
**Learning:** Re-instantiating `Intl.NumberFormat` repeatedly inside loops, components, and format helpers creates significant performance overhead due to the expensive locale compilation process in JavaScript engines.
**Action:** Always extract `Intl.NumberFormat` and `Intl.DateTimeFormat` into a shared module-level cache (e.g., using a Map with `JSON.stringify(options)` as the key) instead of re-instantiating them to improve JavaScript execution performance.
