...
## 2026-03-24 - Fix N+1 Query in Visitor Chats via SQLite Aggregation
**Learning:** In Drizzle/SQLite, we can avoid N+1 queries when fetching aggregate counts and latest rows by using a single `inArray()` query with `GROUP BY` and an aggregate function like `max(createdAt)`. Because of SQLite's bare columns feature, it natively returns the corresponding column values (like `content`) from the exact row containing that maximum value.
**Action:** Use this bulk query pattern (`select`, `inArray`, `groupBy`, `max`) for fetching list items with their related latest child records instead of iterating and fetching with `Promise.all` and `.limit(1)`.
