...
## 2024-05-31 - Fix N+1 queries using inArray and groupBy
**Learning:** Resolving N+1 queries in Drizzle ORM and Cloudflare D1 with Promise.all() leads to significant latency. While db.batch() helps, it has a 100-statement limit. Using inArray() combined with groupBy() and aggregate functions allows retrieving the aggregated metrics in a single query compatible across local environments and D1, by manually mapping the array results back to the objects.
**Action:** When mapping over records to fetch related aggregate data (counts, latest items), use a single inArray() query grouping by the parent ID instead of using Promise.all(map(...)) or db.batch().
