...
## 2026-03-20 - [Batch independent sequential D1 queries via db.batch()]
**Learning:** Cloudflare D1 and Drizzle ORM support `db.batch()` which allows sending an array of independent query statements to the database in a single HTTP request. This fundamentally differs from `Promise.all()` as `Promise.all()` still issues separate HTTP requests under the hood, whereas `db.batch()` minimizes actual network latency.
**Action:** Always group independent sequential Drizzle queries into a single `db.batch()` call to eliminate network roundtrip overhead and significantly improve data loading speeds for routes resolving multiple unrelated queries.