...
## 2026-03-20 - [Eliminate Drizzle N+1 in Cloudflare D1 with inArray and GROUP BY]
**Learning:** Cloudflare D1 and Drizzle ORM do not support resolving related counts within nested loops efficiently, often resulting in developers using `Promise.all` mapping which introduces severe N+1 query bottlenecks.
**Action:** Replace `Promise.all` with a single grouped aggregate query using `inArray()`. In SQLite/D1, to fetch the most recent child record along with its count, use `GROUP BY parent_id`, `count(*)`, and `max(created_at)`. Map the results to a Record object keyed by ID in memory to ensure an O(1) lookup when attaching the aggregated stats to the parent array. This avoids the bottleneck without requiring complex Drizzle subquery syntax.
