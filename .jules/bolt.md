...
## 2025-02-14 - [Batch DB Queries Over Promise.all]
**Learning:** Using `Promise.all` + `map()` with Drizzle ORM array operations resolves into N independent HTTP request calls against Cloudflare D1 per mapped execution, creating sequential/parallel network bottlenecks (N+1 latency) due to the Worker runtime overhead.
**Action:** Always replace independent `db.select()` mappings wrapped in `Promise.all` with `db.batch()` natively when interacting with Cloudflare D1. Bypass tuple-length-type-limitations cleanly utilizing `as any` when querying lists of unknown sizes.
