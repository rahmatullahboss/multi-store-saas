...
## 2024-05-15 - [Efficient Aggregation in SQLite/D1 with Drizzle ORM]
**Learning:** In SQLite/D1, fetching both the aggregate count of related rows and the bare columns of the row representing the maximum timestamp value can be efficiently done using `inArray` alongside `groupBy` and `max`. This natively avoids the need for complex subqueries or costly N+1 individual queries per parent row.
**Action:** When resolving related aggregates (like counts and latest rows) for lists in Drizzle ORM and Cloudflare D1, combine queries using `inArray()` with `groupBy()` to fetch everything in a single optimized HTTP request. Remember to cast aggregate outputs properly using `mapWith(Number)`.
