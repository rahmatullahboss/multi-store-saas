...
## 2026-05-08 - Optimize N+1 Queries in SQLite with Bare Columns
**Learning:** In Drizzle ORM / SQLite / Cloudflare D1, when you need to fetch the latest related row alongside an aggregate count (which usually causes an N+1 query issue in naive loops), SQLite's native support for bare columns in `GROUP BY` clauses allows you to fetch both the `count(*)` and the contents of the latest row (`max(createdAt)`) in a single `inArray` query without complex subqueries. Additionally, remember that Drizzle ORM returns `max()` aggregates on integer timestamps as a numeric epoch in seconds, requiring manual `* 1000` rehydration.
**Action:** Use `inArray` with `GROUP BY` and `max()` instead of `Promise.all` + `.map` in lists to eliminate N+1 bottlenecks when fetching associated counts and the latest item's data.
