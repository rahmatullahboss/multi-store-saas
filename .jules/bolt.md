...
## 2026-03-24 - [Cache Intl.NumberFormat outside React Components]
**Learning:** `Intl.NumberFormat` is incredibly slow to instantiate. While using `useMemo` is common inside React components, many utility functions (like `formatPrice.ts` or `money.ts`) were instantiating new `Intl.NumberFormat` objects on every call. This causes major performance bottlenecks when these utilities are used to map over arrays of data or rendered frequently.
**Action:** Always create a module-level cache (using a `Map`) keyed by locale and `JSON.stringify(options)` for `Intl.NumberFormat` and `Intl.DateTimeFormat` objects. Reuse the instances across utility function calls rather than creating new ones.
