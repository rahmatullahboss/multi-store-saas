...
## 2026-03-24 - [Intl.NumberFormat Caching]
**Learning:** Instantiating `Intl.NumberFormat` instances repeatedly inside loops or render paths creates a significant performance overhead because instantiation is computationally expensive.
**Action:** Use centralized module-level caching (like `getCachedNumberFormat` utilizing a Map and `JSON.stringify` keying) rather than constantly calling `new Intl.NumberFormat()` to dramatically improve frontend formatting performance.
