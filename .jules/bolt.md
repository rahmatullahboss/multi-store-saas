...
## 2026-03-24 - [Cache Intl.NumberFormat Initializations]
**Learning:** `Intl.NumberFormat` instantiation is notoriously slow in V8/JavaScript. Our frontend applications and formatting utilities frequently create new `Intl.NumberFormat` instances in tight loops or repeated renders (like formatting prices in a product grid), causing measurable CPU overhead and rendering delays.
**Action:** Always cache `Intl.NumberFormat` (and `Intl.DateTimeFormat`) instances using a Map. Since the `options` object is flat and scalar, `JSON.stringify(options)` is a safe and highly effective deterministic cache key for memoizing these expensive formatters.
