...
## 2026-07-04 - [Optimize N+1 DB Queries via Grouping]
**Learning:** Using `Promise.all` with a `.map()` to iterate over records and fetch related child records causes massive N+1 connection/network latency on Cloudflare D1. While Drizzle supports `db.batch()`, it has a strict limit and doesn't run locally well.
**Action:** Always extract IDs from a parent array and use a single `inArray()` query combined with `groupBy()` to fetch counts or latest related records. Map the results in-memory rather than relying on simultaneous asynchronous connections.
