...
## 2024-03-05 - Bolt: Optimize Visitor Chats Loader
**Learning:** The admin visitor chats loader was using a naive N+1 `Promise.all` loop to fetch message counts and latest messages for each visitor. While manageable at low volume, this scales poorly. By leveraging `inArray` combined with `GROUP BY` and an aggregate `MAX()` function, SQLite reliably returns the bare column value from the row containing the maximum aggregate, enabling a single efficient query to replace dozens of individual requests.
**Action:** Replace `Promise.all` loops containing sequential database lookups with `inArray` bulk queries and manual in-memory mapping to resolve N+1 latency issues effectively in Drizzle + D1.
