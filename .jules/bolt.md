...
## 2024-05-18 - Optimize Drizzle ORM N+1 with SQLite Native Grouping
**Learning:** In Drizzle ORM with Cloudflare D1/SQLite, you can't easily fetch aggregates + bare columns cleanly using joins or subqueries. Using `Promise.all` + `.map()` creates severe N+1 problems. However, `inArray()` with `GROUP BY` and an aggregate function like `max(createdAt)` allows SQLite to natively return the latest row values without an N+1 loop.
**Action:** Replace `Promise.all` mapped over relations with bulk `inArray()` + `GROUP BY` queries, manually re-mapping the results back in Javascript. Use `new Date(lastActive)` as Drizzle returns timestamps as Numbers when grouped.
