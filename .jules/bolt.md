...
## 2026-03-24 - [Avoid `run_in_bash_session` memory limit truncation during git merge diff]
**Learning:** Trying to replace a large section of code based on a truncated file read results in hallucinated contexts and diff failures.
**Action:** Always use `sed` with tight line constraints (e.g., `sed -n '30,65p' <file>`) to confirm the exact textual context before modifying files.
