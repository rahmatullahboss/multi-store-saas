...
## 2026-03-24 - [Resolve N+1 Queries with inArray and groupBy]
**Learning:** Using `Promise.all` with `.map()` to fetch child records individually for a parent array causes severe N+1 latency, which is slow over a network (e.g. Cloudflare D1).
**Action:** Always extract parent IDs, execute a single query using Drizzle's `inArray()` with `groupBy()`, and perform a single `MAX()` alongside bare columns, mapping the results back in memory.
