...
## 2026-03-24 - [Avoid Promise.all for Drizzle Relationships]
**Learning:** Using `Promise.all(entities.map(async (e) => await db.select().where(eq(e.id))))` to fetch 1:many relations causes a massive N+1 query issue on Cloudflare D1.
**Action:** Always map the IDs first, fetch the related records using `inArray()`, and group the results in-memory. This collapses N+1 database roundtrips into a single efficient query.
