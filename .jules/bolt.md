...
## 2024-11-20 - SQLite Bare Column max() with GROUP BY
**Learning:** In Drizzle ORM / SQLite (including Cloudflare D1), using an aggregate like `max(visitorMessages.createdAt)` alongside bare columns (like `visitorMessages.content`) in a `GROUP BY` query naturally causes SQLite to return the bare column values from the specific row that contains the maximum value. This allows avoiding an expensive subquery or N+1 lookup when fetching the "latest" item for a list of entities.
**Action:** When needing to fetch the latest related record for multiple entities simultaneously (e.g. latest chat message for a list of visitors), use a single `inArray()` + `GROUP BY` + `max(createdAt)` query instead of `Promise.all(map(...))` loops, which create devastating N+1 vulnerabilities.
