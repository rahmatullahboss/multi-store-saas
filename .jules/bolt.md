...
## 2026-05-16 - [Cache Intl.NumberFormat and Intl.DateTimeFormat in React Components]
**Learning:** Instantiating `Intl.NumberFormat` or `Intl.DateTimeFormat` inside render paths or mapping loops is a significant performance bottleneck (e.g., >1ms per call, leading to 1+ second latencies on large lists or charts).
**Action:** Always cache `Intl` object instantiations when used in frontend components. Use centralized utilities (like `getCachedNumberFormat`) to prevent expensive re-instantiations during re-renders or maps.
