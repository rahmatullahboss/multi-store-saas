...
## 2026-03-24 - [Guard Drizzle inArray() Queries]
**Learning:** Using Drizzle ORM's `inArray()` operator with an empty array causes a runtime SQL syntax error.
**Action:** Always wrap `inArray()` queries in an explicit array length check (e.g., `if (ids.length > 0)`) to prevent queries from crashing when there are no parent relations to look up.
