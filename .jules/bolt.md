...
## 2024-04-19 - Optimize Intl.NumberFormat Instantiation
**Learning:** Re-instantiating formatters like `Intl.NumberFormat` is a significant JavaScript performance bottleneck, especially in render loops.
**Action:** Centralized module-level caching for `Intl.NumberFormat` was implemented using a shared `Map` keyed by locale and options to reuse instances.
