...
## 2025-04-14 - [Cache Intl.NumberFormat in Utilities]
**Learning:** `Intl.NumberFormat` instantiation is notoriously slow in JavaScript due to ICU data loading. While React components can use `useMemo` to mitigate this, utility functions (like `formatPrice.ts` and `money.ts`) often instantiate a new formatter on every call. In list views or loops, this causes severe micro-stutters and increased blocking time on the main thread.
**Action:** When working on formatting utilities outside of React components, always implement a centralized module-level cache (e.g., using a `Map` with stringified options as keys) to reuse `Intl.NumberFormat` instances instead of creating new ones per call.
