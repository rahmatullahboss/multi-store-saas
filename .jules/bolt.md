...
## 2024-06-12 - SQLite `max()` behavior with bare columns
**Learning:** In Drizzle ORM / SQLite (and Cloudflare D1), selecting a bare column alongside an aggregate function like `max()` or `min()` in a `groupBy` guarantees that the bare column's value belongs to the row that matched the aggregate value. This avoids complex subqueries and simplifies solving N+1 query problems for fetching the 'latest' items related to a parent entity.
**Action:** When needing the latest or oldest related record for multiple parents (e.g. latest message per visitor), use `inArray()` with `groupBy()` and select the bare data column alongside `max(timestamp_column)`.
