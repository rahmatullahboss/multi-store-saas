...
## 2026-03-24 - [Optimize Cloudflare D1 HTTP Overhead with db.batch]
**Learning:** Using `Promise.all` for concurrent Drizzle ORM queries still creates separate HTTP requests to Cloudflare D1 for each query, causing network overhead. Cloudflare D1 natively supports batching, which Drizzle exposes via `db.batch()`. This combines multiple SQL queries into a single HTTP round-trip payload.
**Action:** Always prefer `db.batch([ db.select... ])` over `Promise.all([ db.select... ])` when executing multiple independent Drizzle ORM statements in Cloudflare Workers to eliminate redundant network latency.
