...
## 2026-03-19 - [SQLite Aggregate Grouping Behavior in Drizzle ORM]
**Learning:** When trying to resolve an N+1 `Promise.all` fetch where we need an aggregate (like `max(createdAt)` or `count()`) *and* the value of another column (like `content`) from the *latest* row, SQLite inherently supports returning the corresponding bare column for the row that generated the max aggregate.
**Action:** Exploit this native behavior in Drizzle ORM by performing a single `inArray` query with a `groupBy`, requesting both the `sql\`max(...)\`` and the raw bare column. This avoids needing complex window functions or secondary fetches to get data attached to the "latest" record.
