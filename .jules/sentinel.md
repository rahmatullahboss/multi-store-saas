...
## 2024-05-02 - [CRITICAL] Fix DOMPurify server-side rendering errors
**Vulnerability:** The application was using custom whitelist-based sanitization in `apps/web/app/utils/sanitize.ts` which is insecure and a known anti-pattern. While trying to replace it with `dompurify` and `jsdom`, it causes a fatal error during production SSR when dev-dependencies are pruned because `jsdom` is only installed as a `devDependency`.
**Learning:** `jsdom` will break production SSR when pruned.
**Prevention:** Use `isomorphic-dompurify` instead which safely handles SSR.
