...
## 2023-10-27 - [HIGH] Fix XSS vulnerability in DC Store ProductPage
**Vulnerability:** The `product.description` field was being rendered directly using `dangerouslySetInnerHTML` in the DC Store `ProductPage.tsx` template without being sanitized. This allowed arbitrary HTML, potentially including malicious scripts, to be executed if a user provided a crafted product description.
**Learning:** Always validate and sanitize user-provided HTML content before rendering it using `dangerouslySetInnerHTML`. Direct rendering without sanitization is a common XSS vector, especially in product descriptions or similar user-generated fields across different templates.
**Prevention:** Consistently import and apply a sanitization utility like `sanitizeHtml` to all `dangerouslySetInnerHTML` assignments involving user-controlled content throughout all templates.
