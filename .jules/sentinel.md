...
## 2025-04-25 - XSS Vulnerability in DC Store Template
**Vulnerability:** The `product.description` field was being rendered directly via `dangerouslySetInnerHTML` in `apps/web/app/components/store-templates/dc-store/pages/ProductPage.tsx` without sanitization.
**Learning:** Store templates migrated from older architectures or mockups may miss global sanitization utilities used in the rest of the application. The `dc-store` template rendered the raw description string directly.
**Prevention:** Always verify that `dangerouslySetInnerHTML` receives content wrapped in `sanitizeHtml` (or equivalent) in all newly created or migrated React storefront templates.
