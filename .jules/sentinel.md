...
## 2025-03-22 - [HIGH] Fix XSS vulnerability in DC Store ProductPage
**Vulnerability:** The `product.description` field was being rendered via `dangerouslySetInnerHTML` directly in the DC Store template without sanitization. If a user provided malicious HTML, it would execute.
**Learning:** Always sanitize user-generated HTML content before rendering it with `dangerouslySetInnerHTML` across all templates.
**Prevention:** Use `sanitizeHtml(product.description || '')` instead of passing the raw string to `__html`.
