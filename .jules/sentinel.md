...
## 2025-03-24 - [HIGH] Fix XSS vulnerability in DC Store product page
**Vulnerability:** The `product.description` field was rendered directly via `dangerouslySetInnerHTML` without sanitization in the DC Store product template.
**Learning:** Even if user input isn't sliced, directly injecting it into `dangerouslySetInnerHTML` is an XSS vector. Always sanitize.
**Prevention:** Consistently use `sanitizeHtml` on any user-provided content before rendering it with `dangerouslySetInnerHTML`, particularly in store templates where descriptions often contain HTML.
