...
## 2025-05-27 - [HIGH] Fix XSS vulnerability in dc-store ProductPage
**Vulnerability:** The `product.description` field in `apps/web/app/components/store-templates/dc-store/pages/ProductPage.tsx` was rendered directly via `dangerouslySetInnerHTML` without any sanitization.
**Learning:** Similar to Eclipse and Luxe Boutique templates, any unsanitized rendering of user-controlled input (like `product.description`) using `dangerouslySetInnerHTML` allows for Stored Cross-Site Scripting (XSS).
**Prevention:** Consistently use `sanitizeHtml(product.description || '')` before rendering any user-generated HTML content using `dangerouslySetInnerHTML` across all templates.
