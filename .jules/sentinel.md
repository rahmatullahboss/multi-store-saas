...
## 2024-11-20 - [XSS Vulnerability in DC-Store Template]
**Vulnerability:** Unsanitized user input (`product.description`) was rendered using `dangerouslySetInnerHTML` in `dc-store`'s `ProductPage.tsx`.
**Learning:** Store templates rendering dynamic content from merchants must sanitize all HTML fields to prevent stored XSS attacks.
**Prevention:** Always use `sanitizeHtml(product.description || '')` when dealing with potentially unsafe HTML content in templates before passing to dangerouslySetInnerHTML.
