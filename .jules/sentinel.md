...
## 2025-03-22 - [HIGH] Fix XSS vulnerability in DC Store product descriptions
**Vulnerability:** The product description in the DC Store product page was rendered via `dangerouslySetInnerHTML` without sanitization.
**Learning:** `dangerouslySetInnerHTML` is widely used across different store templates, and user-provided inputs like product descriptions require explicit sanitization (e.g., using `sanitizeHtml`) to prevent Cross-Site Scripting (XSS).
**Prevention:** Always wrap variables passed into `dangerouslySetInnerHTML` with a robust HTML sanitization utility to ensure all inputs are scrubbed before rendering.
