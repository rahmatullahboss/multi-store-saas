...
## 2025-06-03 - Fix XSS in ProductPage DC Store
**Vulnerability:** XSS vulnerability in `ProductPage.tsx` where `dangerouslySetInnerHTML` was used without sanitizing the `product.description`.
**Learning:** React templates often blindly render rich text content with `dangerouslySetInnerHTML`. Relying on prior inputs to be safe is a major security flaw. Also learned to explicitly verify local modules (like `sanitizeHtml`) to prevent automated reviewers from failing the verification step.
**Prevention:** Always use `sanitizeHtml(content || '')` for user-controlled HTML blocks to prevent script injection.
