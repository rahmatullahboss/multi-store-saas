...
## 2025-04-15 - [HIGH] Fix XSS vulnerability in DC Store product template
**Vulnerability:** The `product.description` field was being rendered via `dangerouslySetInnerHTML` directly in the DC Store product page template without any sanitization. If a user provided HTML with malicious scripts, they would execute in the browser.
**Learning:** Newly ported or added store templates (like DC Store) often miss standard security components (like `sanitizeHtml`) used elsewhere in the codebase, re-introducing raw DOM injection vulnerabilities.
**Prevention:** Always verify that `sanitizeHtml` is imported and used alongside `dangerouslySetInnerHTML` for any user-controlled input (like `product.description`) when creating or porting new components.
