...
## 2025-03-22 - [CRITICAL] Fix XSS vulnerability in dc-store template
**Vulnerability:** The `dc-store` template's `ProductPage.tsx` was rendering raw user-provided `product.description` directly via React's `dangerouslySetInnerHTML` without any sanitization, allowing arbitrary script execution (XSS).
**Learning:** Store templates imported from original themes often bypass the central security practices. Components like `dc-store/pages/ProductPage.tsx` duplicated UI elements but missed the `sanitizeHtml` wrapping present in the shared `ProductPage.tsx`.
**Prevention:** When importing or creating new store templates that render user-generated HTML, always audit `dangerouslySetInnerHTML` usage to ensure `sanitizeHtml` from `~/utils/sanitize` is applied.
