...
## 2024-06-11 - XSS Vulnerability in Product Description
**Vulnerability:** Unsanitized user input in dangerouslySetInnerHTML for product.description in the DC Store template.
**Learning:** User-generated content like product descriptions must always be sanitized before being injected into the DOM, even in templates that might be perceived as 'trusted' or pre-defined.
**Prevention:** Always use the sanitizeHtml utility from ~/utils/sanitize when rendering user-controlled content with dangerouslySetInnerHTML.
