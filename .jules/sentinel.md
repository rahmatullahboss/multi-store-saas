...
## 2025-02-25 - XSS in dc-store Template

**Vulnerability:** Found `dangerouslySetInnerHTML={{ __html: product.description }}` without `sanitizeHtml` in `apps/web/app/components/store-templates/dc-store/pages/ProductPage.tsx`.
**Learning:** Store templates ported from older versions might miss the `sanitizeHtml` wrapper, leaving raw DOM injections for user-controlled input.
**Prevention:** Always check new store templates for `dangerouslySetInnerHTML` usages and ensure `sanitizeHtml` is used.
