...
## 2025-03-24 - [HIGH] Fix XSS vulnerability in DC Store product descriptions
**Vulnerability:** The `product.description` field was being rendered directly via `dangerouslySetInnerHTML` in the DC Store template. If a user provided HTML with malicious scripts, they would execute without sanitization.
**Learning:** Even internal or semi-trusted inputs (like product descriptions) must be sanitized when rendered with `dangerouslySetInnerHTML` to protect against stored XSS attacks, especially across all store template variants.
**Prevention:** Always wrap variables passed to `dangerouslySetInnerHTML` with a robust sanitization function like `sanitizeHtml`, ensuring a fallback for undefined values.
