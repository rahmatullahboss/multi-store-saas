...
**Vulnerability:** Similar to other templates, the `product.description` field in the DC Store template was being rendered via `dangerouslySetInnerHTML` directly without sanitization.
**Learning:** The `sanitizeHtml` utility needs to be applied consistently across all store templates when rendering user-generated content like `product.description`.
**Prevention:** Establish a systematic check or lint rule to ensure `dangerouslySetInnerHTML` usage is always paired with `sanitizeHtml` or a similar established security library for all user-generated fields.
