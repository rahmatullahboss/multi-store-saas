...
## 2025-03-21 - [HIGH] Fix XSS vulnerability in product descriptions
**Vulnerability:** The `product.description` field was being rendered via `dangerouslySetInnerHTML` directly in the DC Store template. If a user provided HTML with malicious scripts, they would execute without sanitization.
**Learning:** When rendering user-generated HTML content (like product.description) for previews, always apply `sanitizeHtml` to prevent malformed, unclosed HTML tags from breaking the layout, and to clean malicious input.
**Prevention:** Avoid rendering raw `product.description` without wrapping the final string in `sanitizeHtml`.
