...
## 2025-02-15 - Prevent XSS in DC Store Product Description
**Vulnerability:** The `product.description` field was directly rendered via `dangerouslySetInnerHTML` in the DC Store template. Unsanitized user input could execute malicious scripts.
**Learning:** All user-provided rich text (like product descriptions) must be sanitized before rendering with `dangerouslySetInnerHTML`.
**Prevention:** Always wrap potentially unsafe user input with `sanitizeHtml()` and provide a fallback empty string (e.g., `sanitizeHtml(product.description || '')`) when rendering HTML.
