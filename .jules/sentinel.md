...
## 2025-02-28 - XSS in Store Template dangerouslySetInnerHTML
**Vulnerability:** Found unescaped user input (`product.description`) passed directly to `dangerouslySetInnerHTML` in `apps/web/app/components/store-templates/dc-store/pages/ProductPage.tsx`.
**Learning:** `dangerouslySetInnerHTML` usage needs to be closely audited in store templates where potentially untrusted data from the DB is rendered.
**Prevention:** Always wrap variables passed to `dangerouslySetInnerHTML` with `sanitizeHtml()`, even if they originate from the application's own database.
