...
## 2024-05-18 - Missing HTML Sanitization in Store Templates
**Vulnerability:** XSS vulnerability found in `dc-store` product page where `product.description` was rendered using `dangerouslySetInnerHTML` without any sanitization.
**Learning:** Store templates rendering user-controlled content like `product.description` need explicit sanitization. While other templates were using `sanitizeHtml`, `dc-store` was missed.
**Prevention:** Always wrap user-controlled fields like `product.description` in `sanitizeHtml()` when rendered via `dangerouslySetInnerHTML` in store templates to avoid XSS vulnerabilities.
