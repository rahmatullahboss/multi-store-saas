...
## 2024-06-13 - Missing sanitizeHtml in DC Store Template
**Vulnerability:** The `product.description` field was being rendered directly using `dangerouslySetInnerHTML` in `apps/web/app/components/store-templates/dc-store/pages/ProductPage.tsx`. This created an XSS risk if the user supplied malicious HTML for the product description.
**Learning:** Even when core templates implement proper sanitization, newer or specific store templates may lack these security measures, leading to regressions in isolated areas.
**Prevention:** Always verify that `dangerouslySetInnerHTML` uses `sanitizeHtml` across all newly created template components, regardless of the folder they reside in.
