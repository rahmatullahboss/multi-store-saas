...
## 2025-03-21 - [HIGH] Fix XSS vulnerability in product description renders
**Vulnerability:** The `product.description` field was rendered using `dangerouslySetInnerHTML` directly in the DC Store template.
**Learning:** Always use `sanitizeHtml` on any user-provided content rendered via `dangerouslySetInnerHTML` in templates.
**Prevention:** Avoid rendering raw `product.description` chunks without wrapping the final string in `sanitizeHtml`.