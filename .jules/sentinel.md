...
## 2025-03-27 - [HIGH] Recurrence of XSS vulnerability in DC Store product previews
**Vulnerability:** Similar to previous templates, the DC Store product template (`DCProductPage`) was rendering `product.description` directly into the DOM using `dangerouslySetInnerHTML` without proper sanitization.
**Learning:** Newly created or migrated templates (like OS2 themes being ported) often re-introduce the same vulnerabilities if security components (`sanitizeHtml`) are not included by default in the template scaffolding.
**Prevention:** Always verify that newly added store templates use `sanitizeHtml` around user-controlled HTML inputs, especially `product.description`. Ensure any generic template boilerplate enforces this practice.
