...
## 2025-06-10 - [HIGH] Fix XSS vulnerability in DC Store Product Page
**Vulnerability:** The `product.description` field was being rendered directly via `dangerouslySetInnerHTML` in the DC Store template without any sanitization.
**Learning:** Always provide a fallback empty string `|| ''` when wrapping potentially undefined user-controlled fields with `sanitizeHtml()` in React components to prevent runtime crashes.
**Prevention:** Wrap user-generated HTML content with `sanitizeHtml(field || '')` before rendering it via `dangerouslySetInnerHTML`.
