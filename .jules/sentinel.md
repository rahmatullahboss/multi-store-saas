...
## 2024-04-26 - [XSS] Unsanitized product descriptions in template
**Vulnerability:** The `dc-store` template was rendering `product.description` directly using `dangerouslySetInnerHTML` without any sanitization.
**Learning:** Store templates ported or created as duplicates often overlook the `sanitizeHtml()` wrapper when calling `dangerouslySetInnerHTML`. This allows XSS if an attacker inputs malicious scripts in product fields.
**Prevention:** Ensure that every usage of `dangerouslySetInnerHTML` throughout the project, especially in template files rendering user-controlled content, wraps the input with `sanitizeHtml()`.
