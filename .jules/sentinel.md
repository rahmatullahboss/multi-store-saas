...
## 2025-02-24 - [Fix XSS Vulnerability in DC Store Template]
**Vulnerability:** Similar to other templates, the `product.description` field was being rendered via `dangerouslySetInnerHTML` directly in the DC Store template. If a user provided malicious scripts, they would execute without sanitization.
**Learning:** We must consistently sanitize all raw text from users before passing them to `dangerouslySetInnerHTML`.
**Prevention:** Using `sanitizeHtml(product.description || '')` before setting it in React DOM node.
