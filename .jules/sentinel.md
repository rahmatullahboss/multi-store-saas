...
## 2024-06-28 - Missing Sanitization in Store Templates
**Vulnerability:** The `product.description` field in the DC Store template was being rendered via `dangerouslySetInnerHTML` directly without sanitization, posing an XSS risk.
**Learning:** Store templates are particularly vulnerable to XSS since user-provided descriptions can contain arbitrary HTML and are rendered dynamically.
**Prevention:** Always wrap dynamically rendered HTML content in `sanitizeHtml(content || '')` before passing it to `dangerouslySetInnerHTML`.
