...
## 2025-02-09 - Missing Sanitization in Store Templates
**Vulnerability:** Found `product.description` rendered raw via `dangerouslySetInnerHTML` in the new `dc-store` template's `ProductPage.tsx`.
**Learning:** When porting or creating new store templates, developers often forget to wrap user-controlled data in `sanitizeHtml()`, leading to raw XSS injections. Store templates are particularly prone to this as they are often duplicated.
**Prevention:** Enforce use of `sanitizeHtml()` consistently across all instances of `dangerouslySetInnerHTML` when rendering potentially dangerous fields (like descriptions) in any template or component.
