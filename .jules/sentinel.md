...
## 2024-05-30 - Missing Sanitization in Store Templates
**Vulnerability:** XSS vulnerability in dc-store ProductPage where user-provided `product.description` was rendered via `dangerouslySetInnerHTML` without sanitization.
**Learning:** While most store templates used `sanitizeHtml` for rendering product descriptions, one newer template omitted it.
**Prevention:** Always verify that every instance of `dangerouslySetInnerHTML` wrapping user-provided content applies `sanitizeHtml` in newly added store templates.
